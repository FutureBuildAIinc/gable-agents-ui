---
title: "External Agents: Claude, ChatGPT, Codex, Cursor, Cowork, Grok"
description: "Connect Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok, or any MCP-compatible host to a hosted agent-native app — then round-trip artifacts back into the running UI with MCP Apps and deep links."
search: "Claude Anthropic ChatGPT OpenAI Claude Code Codex Cursor Claude Cowork Grok xAI MCP Apps agent-native connect local agent tools external agents"
---
...
Every Agent-Native app exposes one canonical MCP URL:

```text
https://<your-app>.agent-native.com/mcp
```

In the app, open **Settings → Integrations** and search for the host name you
already know: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok, or xAI. The one external-host result opens **Settings → MCP** with
the closest guide selected. Copy the **MCP server URL** and paste it into the AI
host. The URL is the deployed app base URL plus `/mcp`.
...
host-specific tabs for Claude, ChatGPT, Cursor, Claude Code, Codex, Grok, and
Other.

<a id="access-tab"></a>

### The in-app MCP setting {#mcp-settings}

Standard app Settings includes an **MCP** tab beside **Integrations**. It shows
the app's canonical MCP URL, an A2A agent card when available, and the same
client setup instructions used by the full connect page. Use **Settings → MCP**
as the in-app starting point, or open `/mcp/connect` directly for the full flow
and static-token fallback.
...
ChatGPT: custom MCP apps require a supported ChatGPT web workspace. If you have
access, open **Settings → Apps → Advanced settings**, enable developer mode when
required, choose **Create app**, paste the same MCP URL, choose OAuth, scan the
tools, sign in with Agent-Native, approve the requested scopes, and enable the
app in a chat. Business, Enterprise, and Edu workspaces support full MCP,
including write actions; Pro supports read and fetch in developer mode. If
**Apps** or **Create app** is missing, your plan or workspace policy does not
allow this setup.
...
External agents get the app's real create/update tools — the same named actions the in-app agent calls — so when a user asks for new content (a design, a deck, a form, a document), author it directly and persist it with those tools instead of delegating the work through `ask_app`. Actions marked `endsTurn: true`, such as an in-app question form that waits for the user's answer in that app's own chat, are hidden from MCP, WebMCP, and A2A unless `mcpTool: true` — see [endsTurn in Defining Actions](/docs/actions-defining). Tool inputs and results carry up to 500,000 characters on WebMCP, so a whole screen, deck, or document fits in one write — see [WebMCP](/docs/webmcp). Page-local WebMCP calls carry the browser tab id (the same `getBrowserTabId()` value the UI uses — see [Context Awareness](/docs/context-awareness)), so per-tab navigation and selection resolve to the tab the agent is driving, even when hidden.
...
- [WebMCP](/docs/webmcp) — page-local tools, the 500,000-character input/result limit, and the browser tab id header.
...
- [Actions](/docs/actions-overview) — defining actions, `publicAgent`, GET / `readOnly`.
