...
<WireframeBlock id="doc-block-5ro1zq">
  <Screen
    surface="desktop"
    html={
...
असिंक्रोनस स्क्रीन रिकॉर्डिंग, कैलेंडर से सिंक किए गए मीटिंग नोट्स और पुश-टू-टॉक वॉइस डिक्टेशन एक ही ऐप में — एजेंट हर सतह पर प्रथम श्रेणी का संपादक है, और रिकॉर्डिंग, मीटिंग व डिक्टेशन आपके हैं, किसी SaaS विक्रेता के नहीं। Clips साझा रिकॉर्डिंग को एजेंट के लिए पढ़ने योग्य भी बनाता है: किसी एजेंट में सार्वजनिक Clips शेयर लिंक या अस्थायी **एजेंट के लिए कॉपी करें** लिंक पेस्ट करें, और वह प्रतिलेख को टेक्स्ट के रूप में "सुन" तथा टाइमस्टैम्प वाले स्क्रीन फ़्रेम को छवियों के रूप में "देख" सकता है — कच्चे वीडियो की आवश्यकता नहीं। फ़्रेम देखना किसी भी छवि-सक्षम एजेंट (ChatGPT, Claude Code, Cursor, Codex) में काम करता है; केवल-टेक्स्ट वेब चैट को भी पूरा प्रतिलेख मिलता है और वह आपके अपलोड किए फ़्रेम का उपयोग कर सकता है।

<Diagram id="doc-block-1i8i9bu" title={"कैप्चर करना, प्रतिलेखन करना, पुन: उपयोग करना"} summary={"एक पुस्तकालय में तीन कैप्चर प्रकार की भूमि; एजेंट प्रतिलेखन, शीर्षक और सारांश प्रस्तुत करता है, फिर प्रत्येक प्रतिलेख खोजने योग्य और साझा करने योग्य होता है।"}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">screen recording</div>
    <div class="diagram-node">कैलेंडर मीटिंग</div>
    <div class="diagram-node">Fn दबाकर डिक्टेशन</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    एक library<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">शीर्षक · सारांश · अध्याय</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">खोजें</div>
    <div class="diagram-pill">साझा करें</div>
...
- **सार्वजनिक Loom वीडियो आयात करें।** वेब पर **रिकॉर्ड → Loom आयात करें** चुनें, फिर Loom का शेयर या एम्बेड URL पेस्ट करें। Clips आपकी लाइब्रेरी में एक कॉपी सहेजता है और उपलब्ध होने पर सार्वजनिक ट्रांसक्रिप्ट आयात करता है।
...
- **Slack में सार्वजनिक क्लिप का पूर्वावलोकन करें** चलाने योग्य वीडियो अनफ़र्ल के साथ, जब
...
<Endpoint id="doc-block-1gpqb33" title={"एजेंट संदर्भ प्रविष्टि बिंदु"} method="GET" path="/api/agent-context.json" summary={"किसी साझा क्लिप का संक्षिप्त, एजेंट-पठनीय विवरण"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-19atfxz" title={"टाइमस्टैम्प्ड प्रतिलेख"} method="GET" path="/api/agent-transcript.json" summary={"किसी साझा क्लिप के लिए टाइमस्टैम्प्ड ट्रांसक्रिप्ट सेगमेंट"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1b3isws" title={"टाइमस्टैम्प पर फ़्रेम"} method="GET" path="/api/agent-frame.jpg" summary={"एक JPEG फ़्रेम मूल-वीडियो टाइमस्टैम्प पर वीडियो से निकाला गया"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
2. **उत्पादन स्थिति कॉन्फ़िगर करें।** एक स्थायी `DATABASE_URL`, सामान्य सेट करें
   ...
   वीडियो भंडारण प्रदाता। Builder.io कनेक्ट (free tier includes video storage and AI credits) सबसे आसान भंडारण पथ है; के लिए
...
6. **वैकल्पिक एकीकरण कनेक्ट करें।** Google Calendar मीटिंग टैब को शक्ति प्रदान करता है,
   `GEMINI_API_KEY` या Builder.io कनेक्ट (free tier includes AI credits) पावर ट्रांसक्रिप्ट क्लीनअप और शीर्षक,
...
प्रत्येक एजेंट-कॉल करने योग्य ऑपरेशन `templates/clips/actions/` में एक TypeScript फ़ाइल है, जो `POST /_agent-native/actions/:name` पर ऑटो-माउंटेड है और CLI से `pnpm action <name>` के रूप में चलने योग्य है। लगभग 100 actions हैं; उपयोगी समूह:
