...
<WireframeBlock id="doc-block-hbyzqg">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>エンジニアリング Clips</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>OKR レビュー</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>オンボーディングフロー</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Bug 再現</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>エージェント可読</span><span>文字起こし + フレームを共有リンク用に準備</span><div style='flex:1'></div><button>共有</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>文字起こし検索</strong><div class='wf-box'>一致した章 03:12 · ロールアウトリスクと担当者引き継ぎ</div><div class='wf-box'>会議の要約とアクション項目</div></div></div>"
    }
  />
</WireframeBlock>
...
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">画面録画</div>
    <div class="diagram-node">カレンダー会議</div>
    <div class="diagram-node">Fn 長押しディクテーション</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    1 つのライブラリ<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">タイトル · 要約 · 章</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">検索</div>
    <div class="diagram-pill">共有</div>
    <div class="diagram-pill">エージェント可読 links</div>
  </div>
</div>
```
...
<Endpoint id="doc-block-4qfxj8" title={"エージェントコンテキストのエントリポイント"} method="GET" path="/api/agent-context.json" summary={"エージェントが読み取り可能な共有クリップのコンパクトな説明"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-155lcpd" title={"タイムスタンプ付きトランスクリプト"} method="GET" path="/api/agent-transcript.json" summary={"共有クリップのタイムスタンプ付きトランスクリプトセグメント"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-12ygfm9" title={"タイムスタンプのフレーム"} method="GET" path="/api/agent-frame.jpg" summary={"元のビデオのタイムスタンプでビデオから抽出された JPEG フレーム"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
1. **ビデオ ストレージ (必須)。** オンボーディング ウィザードを通じてストレージ バックエンドに接続します。最も簡単なパスは Builder.io (free tier includes video storage and AI credits、ワンクリック) です。セルフホスト型ストレージの場合は、`S3_ENDPOINT`、`S3_BUCKET`、`S3_ACCESS_KEY_ID`、`S3_SECRET_ACCESS_KEY`、およびオプションで `S3_REGION` および `S3_PUBLIC_BASE_URL` を設定します。 Cloudflare R2 と DigitalOcean Spaces は、`R2_*` プレフィックスが付いた同じ環境変数を使用します。
...
1. **アプリを作成します。**
   ...
   ```bash
   npx --yes @agent-native/core@latest create my-clips --standalone --template クリップ
   CD マイクリップ
   pnpm インストール
   ```
...
2. **運用状態を構成します。** 永続的な `DATABASE_URL` (通常) を設定します。
   ...
   ビデオストレージプロバイダー。 Builder.io Connect (free tier includes video storage and AI credits) は最も簡単なストレージ パスです。
...
6. **オプションの統合を接続します。** Google Calendar は [会議] タブを強化します。
   `GEMINI_API_KEY` または Builder.io Connect (free tier includes AI credits) はトランスクリプトのクリーンアップとタイトルを強化します。
...
<DataModel
  ...
  entities={[
    ...
    },
    {
      ...
      note: "ライブラリとトランスクリプトビューが高速にレンダリングされるように分割する",
      fields: [
        {
          name: "recording_id",
          type: "text",
