---
title: "外部エージェント: Claude、ChatGPT、Codex、カーソル、Cowork、Grok"
description: "Claude、ChatGPT、Codex、Cursor、Claude Cowork、Grok、または MCP 互換ホストをホストされたエージェント ネイティブ アプリに接続し、MCP アプリとディープ リンクを使用してアーティファクトを実行中の UI に往復させます。"
search: "Claude Anthropic ChatGPT OpenAI Claude コード Codex カーソル Claude Cowork Grok xAI MCP アプリ エージェントネイティブ接続 ローカル エージェント ツール 外部エージェント"
---
...
<Diagram id="doc-block-36uhi0" title={"外部エージェントの往復"} summary={"外部ホストは MCP 経由でツールを呼び出します。アプリはアーティファクトと開くリンクを返します。これをクリックすると、ブラウザー セッションが解決され、実行中の UI 内のアーティファクトがフォーカスされます。リンクには特権状態がありません。"}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    外部ホスト<br /><small class="diagram-muted"
      >Claude &middot; ChatGPT &middot; Codex &middot; Cursor</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">MCP ツール call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    アプリが成果物を生成<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>ユーザーがリンクをクリック</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
...
- **Agent-Native チャットの背後にある独自のランタイム:** 別のフレームワークで構築されたエージェントが `<AssistantChat runtime={...}>` を強化する必要がある場合は、[Agent Surfaces](/docs/agent-surfaces#byo-agent) および [Native チャット UI](/docs/native-chat-ui#byo-agent-runtimes) を参照してください。
...
- **ローカル カスタム サブエージェント:** エージェント ネイティブ ワークスペース自体内にカスタム エージェント プロファイルが必要な場合は、[エージェント リソース](/docs/agent-resources) を使用します。

## 簡単なセットアップ {#easy-setup}

すべての Agent-Native アプリには、正規の MCP URL があります。

```text
https://<your-app>.agent-native.com/mcp
```

アプリで **Settings → Integrations** を開き、すでに知っているホスト名を検索
します。Claude、Claude Cowork、Claude Code、Anthropic、ChatGPT、OpenAI、
Codex、Grok、xAI のいずれでも検索できます。外部ホストの検索結果は 1 件だけ
表示され、最も近いガイドを選択した状態で **Settings → MCP** が開きます。
**MCP server URL** をコピーして AI ホストに貼り付けます。URL はデプロイ済み
アプリのベース URL に `/mcp` を付けたものです。
...
`https://&lt;app&gt;/mcp/connect` とコピー可能な URL および
Claude、ChatGPT、カーソル、Claude コード、Codex、Grok、その他のホスト固有のタブ。

<a id="access-tab"></a>

### アプリ内の MCP 設定 {#mcp-settings}

標準アプリの Settings には **Integrations** の横に **MCP** タブがあります。
アプリの正規 MCP URL、利用可能な場合は A2A エージェント カード、完全な接続ページと
同じ設定ガイドを表示します。アプリ内の開始点として **Settings → MCP** を使うか、
完全なフローとトークンのフォールバックには `/mcp/connect` を直接開きます。
...
ChatGPT: カスタム MCP アプリには、対応する ChatGPT web ワークスペースが必要です。
アクセスできる場合は **Settings → Apps → Advanced settings** を開き、必要に応じて
開発者モードを有効にし、**Create app** を選びます。同じ MCP URL を貼り付け、OAuth
を選択してツールをスキャンし、Agent-Native にサインインして要求されたスコープを承認し、
チャットでアプリを有効にします。Business、Enterprise、Edu のワークスペースは書き込み
アクションを含む完全な MCP に対応します。Pro は開発者モードで読み取りと取得に対応します。
**Apps** または **Create app** が表示されない場合、プランまたはワークスペースのポリシーで
この設定が許可されていません。
...
`reconnect` は、指定されたホストおよび選択されたクライアント (コネクタ名に関係なく、URL で一致) について、URL が `/mcp` で終わる MCP 構成エントリを検索し、インストールされている skills に手を触れたり、完全なインストール フローを再実行したりすることなく、認証マテリアルを更新または置換します。ベースアプリ URL (例: `https://plan.agent-native.com`) を渡します — `/mcp` サフィックスが推測されます。認証とツールの読み込みはクライアントごとに行われるため、後でそのクライアントを再起動/再読み込みします。 Codex は、新しくロードされたツールが表示される前に、新しいセッションが必要です。
...
リモート OAuth URL を直接追加できない MCP クライアントの場合は、ブラウザでアプリを開いて、**接続** アフォーダンス (`https://&lt;app&gt;/mcp/connect` で提供) を使用します。ログインした状態で、**接続 / 認証** をクリックします。このページでは、検出されたエージェントを構成するワンクリックのディープ リンク、またはすぐに貼り付けられる `.mcp.json` ブロックのいずれかを提供します。
...
<Diagram id="doc-block-1m097be" title={"2 つのカタログ層"} summary={"すべての呼び出し元はデフォルトでコンパクト層を取得します。 ~105 ツールの全表面はオプトインのみです。ツール検索はギャップを埋めるため、本当に隠されたものは何もありません。"}>
...
デフォルトでは、接続されているすべてのエージェントに、厳選された小規模なカタログが表示されます (約 20 ～ 30 個に対し、フル サーフェスでは約 105 個)。アプリは明示的な `connectorCatalog` を管理するか、認証済み読み取りポリシーを有効にできます。
...
これは一般的なポリシーの例です。Analytics は意図的に `authenticatedReads: "off"` を設定し、レビュー済みの `connectorCatalog` アクションだけを公開します。
...
`resolveExecutionContext` を指定するアプリは、このデフォルトの ID 判定を明示的に置き換えます。そのリゾルバーはプリンシパルを返す前に、未検証の呼び出し元、ゲスト、外部メンバー、未接続のワークスペースを独自に拒否する必要があります。

検証済みの Slack DM は、エージェントを実行する前に既存の Agent-Native 組織メンバーと照合され、ワークスペース/ユーザーの ID リンクとして保存されます。生成されたユーザー/組織コンテキストには、そのユーザーのリソース、指示、スキルが読み込まれます。デフォルトでは、プロフィールの取得に成功しても、メールアドレスがない、またはまだ組織メンバーではないワークスペース メンバーは拒否されます。アプリは `allowAnonymousOrgScopedSlackDm: true` を明示的に設定することで、それらのメンバーを組織スコープの匿名サービスプリンシパルとして実行できます。これは共有チャンネルと同じ組織全体の可視性レベルですが、ユーザーの非公開データには決してアクセスしません。エージェントに表示される注記と、個人アクセスの取得方法を説明する 1 回限りの Slack 通知が付きます。この opt-in は、プロフィール取得の失敗、未検証の呼び出し元、ゲストや外部 (Slack Connect) メンバー、組織に接続されていないワークスペースを許可しません。それらには丁寧な拒否応答を返します。Slack の共有チャンネルはサービスプリンシパルを使い、参加者の個人的な権限を借用しません。管理対象 OAuth と生成される Slack アプリ マニフェストは、bot スコープ `users:read.email` を要求します。新たに追加されたスコープを付与するには、既存のインストールを再接続/再インストールする必要があります。従来の bot トークンによるインストールでは、Slack でスコープを手動追加してください。このスコープがない場合、アプリが `allowAnonymousOrgScopedSlackDm` を明示的に有効にしない限り、DM は拒否されます。
...
外部エージェントはアプリの実際の作成・更新ツール ― アプリ内エージェントが呼び出すのと同じ名前付きアクション ― を利用できます。そのため、ユーザーが新しいコンテンツ(デザイン、デッキ、フォーム、ドキュメントなど)を求めた場合は、`ask_app` に作業を委任するのではなく、直接それを作成し、それらのツールで保存してください。`endsTurn: true` が付いたアクション ― そのアプリ自身のチャットでユーザーの回答を待つアプリ内質問フォームなど ― は、`mcpTool: true` でない限り MCP、WebMCP、A2A から隠されます ― 「[アクションの定義における endsTurn](/docs/actions-defining)」を参照してください。ツールの入力と結果は WebMCP 上で最大 500,000 文字まで扱えるため、画面、デッキ、ドキュメント全体を一度の書き込みに収めることができます ― 「[WebMCP](/docs/webmcp)」を参照してください。ページローカルの WebMCP 呼び出しはブラウザのタブ ID(UI が使うのと同じ `getBrowserTabId()` の値 ― 「[コンテキスト認識](/docs/context-awareness)」を参照)も伝えるため、タブごとのナビゲーションや選択状態は、非表示であってもエージェントが操作しているタブに解決されます。
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | エージェント ループ | 自然言語タスクをそのアプリのアプリ内エージェントにルーティングします。長時間の処理では、`ask_app_status` でポーリングするための永続的な `taskHandle` と従来の `taskId` を返します |
...
### 長時間実行されるエージェント リクエスト

`ask_app` が `taskHandle` を返した場合は、その不透明な値を変更せずに
`ask_app_status` へ渡してください。呼び出し間でアプリ検出が変わっても、元の
タスクルートが保持されます。移行期間中は従来の `app` と `taskId` 引数も引き続き
使用できます。

MCP ホストとゲートウェイは、単一の長いツール呼び出しタイムアウトを共有していません。5 分の上限を課すものもあれば、設定可能なものもあります。10 分間ブロックする `tools/call` を前提に外部エージェント連携を設計しないでください。 `ask-agent` と `ask_app` はホストされている処理をすぐに確認応答し、インライン待機が期限切れになると永続的な `taskHandle` と従来の `taskId` を返します。返された `poll.arguments`、特に `taskHandle` を変更せずに再利用して `ask_app_status` でポーリングしてください。結果にはホストにとって扱いやすい間隔として `pollAfterMs` が含まれます。タスクは最初のレスポンスがタイムアウトする前に作成されている必要があるため、長い呼び出しの後にのみ返されるポーリング URL は信頼できる復旧手段ではありません。
...
返されたものとまったく同じ `poll.arguments`（存在する場合は `taskHandle` を含む）で
`ask_app_status` を再試行してください。ハンドルのない従来の結果では、同じ `app` と
`taskId` を再利用します。送信により
...
<Endpoint id="doc-block-id82v1" method="GET" path="/_agent-native/open" summary={"ディープリンクオープンルート — ブラウザー UI をレコードに集中させます"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
`GET` + `readOnly` は、アクションの副作用を発生させず、画面更新変更イベントの対象外に保ちます。 `publicAgent` は **明示的なオプトイン** です。パブリック Web ルートは、MCP/A2A への公開を決して意味しません。 [Actions](/docs/actions-overview)を参照してください。デザイン/コンテンツの取り込み actions MUST は **ライブ** 状態 (古い DB スナップショット列ではなく、Yjs 共同ドキュメント) を読み取るため、外部エージェントはユーザーが実際に画面上に表示しているものを確認します。コンテンツの `pull-document` は、開いているライブ コラボ セッションを最初に SQL にフラッシュします。デザインの `get-design-snapshot` は、ライブ Yjs ファイルの内容とユーザーが解決した調整値を返します。
...
- 既存の `/mcp` エンドポイントは、他のベアラーと同様にそのトークンを受け入れます ([MCP Protocol](/docs/mcp-protocol) を参照)。新しいエンドポイントや新しいトランスポートはありません。
...
- [WebMCP](/docs/webmcp) ― ページローカル ツール、入力・結果の 500,000 文字制限、およびブラウザ タブ ID ヘッダー。
...
- [Actions](/docs/actions-overview) — actions、`publicAgent`、GET / `readOnly` を定義します。
