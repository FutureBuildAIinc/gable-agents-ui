...
<WireframeBlock id="doc-block-4t9ejf">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:12px;padding:16px;min-height:530px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Q3 取締役会アップデート</h1><span class='wf-pill accent'>タイトルスライド</span><div style='flex:1'></div><button>Preview</button><button>Present</button><button class='primary'>共有</button></div><main style='display:grid;grid-template-columns:1fr 220px;gap:12px;flex:1;min-height:0'><section class='wf-card' style='display:flex;align-items:center;justify-content:center;text-align:center;padding:36px'><div><strong style='font-size:28px'>Q3 取締役会アップデート</strong><br/><small>Maya Chen · 最高経営責任者</small><div style='height:46px'></div><span class='wf-pill'>プロダクトの勢い</span></div></section><section style='display:flex;flex-direction:column;gap:10px'><div class='wf-card'><strong>スライドのアウトライン</strong><div class='wf-box'>1 タイトル</div><div class='wf-box'>2 アジェンダ</div><div class='wf-box'>3 指標</div><div class='wf-box'>4 出荷済み</div></div><div class='wf-card' style='flex:1'><strong>スピーカーノート</strong><p class='wf-muted' style='margin:8px 0 0'>ローンチ進捗とリテンションのストーリーで始める。</p></div></section></main><div style='display:grid;grid-template-columns:repeat(5,1fr);gap:8px'><div class='wf-box'>1 タイトル</div><div class='wf-box'>2 アジェンダ</div><div class='wf-box'>3 指標</div><div class='wf-box'>4 出荷済み</div><div class='wf-box'>5 リスク</div></div></div>"
    }
  />
</WireframeBlock>
...
<Diagram id="doc-block-1lrfamx" title={"デッキへのプロンプト"} summary={"デッキを要求すると、エージェントは CLI から呼び出すことができるのと同じアクションを通じて、スライドを一度に 1 つずつストリーミングします。"}>

```html
<div class="diagram-flow">
  <div class="diagram-node">
    プロンプト<br /><small class="diagram-muted">"10-slide pitch deck"</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Agent</span
    ><small class="diagram-muted">レイアウトを選択</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">create-deck</div>
    <div class="diagram-pill">add-slide &#215; n</div>
    <small class="diagram-muted">並列、ストリーミング</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>decks (SQL)</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">エディターがライブでレンダリング</div>
</div>
```
...
<DataModel
  ...
  entities={[
    ...
    },
    {
      ...
      note: "永続化されたパブリック共有リンクのスナップショット",
      fields: [
        {
          name: "token",
          type: "text",
...
デッキとスライドの作成、編集、削除には `create-deck`、`add-slide`、`update-slide`、`delete-deck` などの共有 actions を使用します。これらはエージェントと UI の両方から呼び出せます。`templates/slides/server/routes/api/` の REST ルートは、アップロード、ストリーミング、エクスポートなどルート固有の処理に限定されています。
