...
<WireframeBlock id="doc-block-1jhfkqi">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Clips de engenharia</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Revisão de OKRs</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Fluxo de integração</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Reprodução de bug</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>Legível por agentes</span><span>Transcrição + frames prontos para links de compartilhamento</span><div style='flex:1'></div><button>Compartilhar</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>Busca em transcrições</strong><div class='wf-box'>Capítulo encontrado 03:12 · riscos de rollout e repasse de proprietário</div><div class='wf-box'>Resumo da reunião e ações</div></div></div>"
    }
  />
</WireframeBlock>

Gravações de tela assíncronas, notas de reuniões sincronizadas com o calendário e ditado por pressionar para falar reunidos em um só aplicativo — o agente é um editor de primeira classe em todas as superfícies, e as gravações, reuniões e ditados são seus, não de um fornecedor de SaaS. O Clips também torna as gravações compartilhadas legíveis por agentes: cole um link público de compartilhamento do Clips ou um link temporário **Copiar para o agente** em um agente, e ele poderá "ouvir" a transcrição como texto e "ver" quadros de tela com marcação de tempo como imagens, sem precisar do vídeo bruto. A visualização de quadros funciona em qualquer agente com capacidade de imagem (ChatGPT, Claude Code, Cursor, Codex); chats web somente de texto ainda recebem a transcrição completa e podem usar um quadro que você enviar.

<Diagram id="doc-block-1lenkc" title="Capture, transcreva, reutilize" summary={"Três tipos de captura pousam em uma biblioteca; o agente transcreve, titula e resume, então cada transcrição pode ser pesquisada e compartilhada."}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">Gravação de tela</div>
    <div class="diagram-node">Reunião de calendário</div>
    <div class="diagram-node">Ditado segurando Fn</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Uma biblioteca<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">título · resumo · capítulos</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">Pesquisar</div>
    <div class="diagram-pill">Compartilhar</div>
    <div class="diagram-pill">Legível por agentes links</div>
  </div>
</div>
```
...
- **Visualize clipes públicos no Slack** com uma prévia de vídeo reproduzível depois que
...
<Endpoint id="doc-block-1ucln5v" title="Ponto de entrada do contexto do agente" method="GET" path="/api/agent-context.json" summary={"Descrição compacta e legível pelo agente de um clipe compartilhado"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1um62lb" title={"Transcrição com carimbo de data/hora"} method="GET" path="/api/agent-transcript.json" summary={"Segmentos de transcrição com carimbo de data/hora para um clipe compartilhado"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-e2ydkl" title="Quadro em um carimbo de data/hora" method="GET" path="/api/agent-frame.jpg" summary={"Um quadro JPEG extraído do vídeo em um carimbo de data/hora do vídeo original"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
2. **Configurar o estado de produção.** Definir um `DATABASE_URL` persistente, o normal
   ...
   provedor de armazenamento de vídeo. Builder.io Connect (free tier includes video storage and AI credits) é o caminho de armazenamento mais fácil; para
...
6. **Conecte integrações opcionais.** Google Calendar alimenta a guia Reuniões,
   `GEMINI_API_KEY` ou Builder.io Connect (free tier includes AI credits) possibilitam a limpeza de transcrições e títulos,
...
<DataModel
  id="doc-block-u84k6r"
  title="Modelo de dados principais de clipes"
  summary={
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Privacidade: senha/expiração",
        },
      ],
    },
    {
      ...
      note: "Divida para que as visualizações da biblioteca e da transcrição sejam renderizadas rapidamente",
      fields: [
        {
          name: "recording_id",
          type: "text",
...
Cada operação que pode ser chamada pelo agente é um arquivo TypeScript em `templates/clips/actions/`, montado automaticamente em `POST /_agent-native/actions/:name` e executável a partir de CLI como `pnpm action <name>`. Existem cerca de 100 actions; os agrupamentos úteis:
