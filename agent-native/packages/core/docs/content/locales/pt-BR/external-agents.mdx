---
title: "Agentes Externos: Claude, ChatGPT, Codex, Cursor, Cowork, Grok"
description: "Conecte Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok ou qualquer host compatível com MCP a um aplicativo nativo de agente hospedado e, em seguida, faça o retorno dos artefatos ao UI em execução com aplicativos MCP e links diretos."
search: "Claude Anthropic ChatGPT OpenAI Claude Código Codex Cursor Claude Cowork Grok xAI MCP Aplicativos agente nativo conectar ferramentas de agente local agentes externos"
---
...
<Diagram id="doc-block-13fhjp1" title="A viagem de ida e volta do agente externo" summary={"Um host externo chama uma ferramenta por MCP; o aplicativo retorna um artefato mais um link Abrir. Clicar nele resolve a sessão do navegador e concentra o artefato na UI em execução – o link não carrega nenhum estado privilegiado."}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    Host externo<br /><small class="diagram-muted"
      >Claude &middot; ChatGPT &middot; Codex &middot; Cursor</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Ferramenta MCP call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    O app produz um artefato<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>Usuário clica no link</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
...
- **Subagentes personalizados locais:** use [Recursos do Agente](/docs/agent-resources) quando desejar perfis de agentes personalizados dentro do próprio espaço de trabalho nativo do agente.

## Configuração fácil {#easy-setup}

Cada aplicativo Agent-Native expõe uma URL MCP canônica:

```text
https://<your-app>.agent-native.com/mcp
```

No aplicativo, abra **Settings → Integrations** e pesquise pelo nome de host que
você já conhece: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok ou xAI. O único resultado de host externo abre **Settings → MCP**
com o guia mais adequado selecionado. Copie a **MCP server URL** e cole-a no
host de IA. A URL é a URL base do aplicativo implantado seguida de `/mcp`.
...
`https://&lt;app&gt;/mcp/connect` com o URL copiável e
guias específicas do host para Claude, ChatGPT, Cursor, Claude Code, Codex, Grok e Outros.

<a id="access-tab"></a>

### A configuração MCP no aplicativo {#mcp-settings}

As configurações padrão do aplicativo incluem uma aba **MCP** ao lado de
**Integrations**. Ela mostra a URL MCP canônica do aplicativo, um cartão de agente A2A
quando disponível e os mesmos guias da página de conexão completa. Use **Settings → MCP**
como ponto de partida ou abra `/mcp/connect` diretamente para o fluxo completo e o fallback
de token.
...
ChatGPT: aplicativos MCP personalizados exigem um espaço de trabalho web do ChatGPT
compatível. Se você tiver acesso, abra **Settings → Apps → Advanced settings**, ative o modo
de desenvolvedor quando necessário, escolha **Create app**, cole a mesma URL MCP, escolha
OAuth, faça a verificação das ferramentas, entre com sua conta Agent-Native, aprove os
escopos solicitados e ative o aplicativo em um chat. Os espaços Business, Enterprise e Edu
oferecem MCP completo, incluindo ações de escrita; o Pro oferece leitura e busca no modo de
desenvolvedor. Se **Apps** ou **Create app** não aparecer, seu plano ou a política do espaço
de trabalho não permite esta configuração.
...
instantâneos. Os escopos `mcp:read` / `mcp:write` / `mcp:apps` / `offline_access` são os mesmos
descritos abaixo em
[OAuth remoto padrão do MCP](#standard-oauth).
...
`reconnect` encontra qualquer entrada de configuração MCP cujo URL termina em `/mcp` para o host fornecido e o cliente selecionado (correspondente a URL, independentemente do nome do conector) e, em seguida, atualiza ou substitui o material de autenticação sem tocar no skills instalado ou executar novamente o fluxo de instalação completo. Passe o aplicativo base URL (por exemplo, `https://plan.agent-native.com`) — o sufixo `/mcp` é inferido. A autenticação e o carregamento da ferramenta são por cliente, portanto, reinicie/recarregue esse cliente posteriormente; Codex precisa de uma nova sessão antes que as ferramentas recém-carregadas apareçam.
...
| `offline_access` | Acesso ao token de atualização para manter o host conectado                                 |

Quando o cliente não solicita nenhum escopo explícito, o aplicativo concede todos os quatro para que o conector se comporte como o fluxo do Connect autorizado pelo navegador e possa atualizar o acesso sem outro login. Mantenha a página Connect do token de portador e o substituto `npx @agent-native/core@latest connect --token <token>` para desenvolvedores locais, hosts substitutos e clientes onde você precisa de um bloco de configuração pronto para colar.
...
<Diagram id="doc-block-x64uzj" title={"Duas camadas de catálogo"} summary={"Cada chamador obtém a camada compacta por padrão; a superfície completa de aproximadamente 105 ferramentas é apenas opcional. a pesquisa de ferramentas preenche a lacuna para que nada fique realmente oculto."}>
...
Este é um exemplo genérico da política. O Analytics define intencionalmente `authenticatedReads: "off"` e expõe somente as actions revisadas de `connectorCatalog`.
...
Um aplicativo que fornece `resolveExecutionContext` substitui explicitamente essa cadeia de identidade padrão. O resolver deve rejeitar de forma independente autores não verificados, convidados, membros externos e workspaces não vinculados antes de retornar um principal.

Antes de o agente ser executado, uma DM verificada do Slack é associada a um membro existente da organização Agent-Native e o vínculo de identidade entre workspace e usuário é salvo. O contexto resultante de usuário/organização carrega os recursos, as instruções e as skills desse usuário. Por padrão, membros do workspace cujo perfil foi carregado, mas cujo e-mail está ausente ou que ainda não pertencem à organização, são recusados. Os aplicativos podem definir explicitamente `allowAnonymousOrgScopedSlackDm: true` para executar esses membros como um principal de serviço anônimo com escopo da organização — o mesmo nível de visibilidade para toda a organização usado pelos canais compartilhados, mas nunca acesso privado do usuário — com uma observação visível ao agente e um aviso único no Slack explicando como obter acesso pessoal. Essa opção nunca admite falhas no carregamento do perfil, autores não verificados, convidados, membros externos (Slack Connect) ou workspaces não conectados a uma organização; esses casos recebem uma resposta educada de recusa. Canais compartilhados do Slack usam um principal de serviço e não herdam as permissões privadas de um participante. O OAuth gerenciado e o manifesto gerado do aplicativo Slack solicitam o escopo de bot `users:read.email`. Instalações existentes precisam ser reconectadas/reinstaladas para conceder um escopo recém-adicionado; instalações antigas com token de bot precisam adicioná-lo manualmente no Slack. Sem ele, as DMs são recusadas, a menos que o aplicativo habilite explicitamente `allowAnonymousOrgScopedSlackDm`.
...
Os agentes externos recebem as ferramentas reais de criação/atualização do aplicativo — as mesmas ações nomeadas que o agente interno chama — então, quando um usuário pedir novo conteúdo (um design, uma apresentação, um formulário, um documento), crie-o diretamente e salve-o com essas ferramentas em vez de delegar o trabalho por meio do `ask_app`. Ações marcadas com `endsTurn: true`, como um formulário de perguntas interno que espera a resposta do usuário no chat do próprio aplicativo, ficam ocultas do MCP, WebMCP e A2A a menos que `mcpTool: true` — veja [endsTurn em Definindo Actions](/docs/actions-defining). Entradas e resultados de ferramentas suportam até 500.000 caracteres no WebMCP, de modo que uma tela, apresentação ou documento inteiro cabe em uma única gravação — veja [WebMCP](/docs/webmcp). Chamadas WebMCP locais à página carregam o id da aba do navegador (o mesmo valor de `getBrowserTabId()` que a UI usa — veja [Consciência do Contexto](/docs/context-awareness)), de modo que a navegação e a seleção por aba são resolvidas para a aba que o agente está controlando, mesmo quando oculta.
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | loop de agente     | encaminha uma tarefa em linguagem natural para o agente no aplicativo desse aplicativo; trabalho longo retorna um `taskHandle` durável e um `taskId` legado para consulta com `ask_app_status` |
...
### Solicitações de agente de longa duração

Quando `ask_app` retornar um `taskHandle`, passe esse valor opaco sem alterações
para `ask_app_status`; ele preserva a rota original da tarefa mesmo que a
descoberta de aplicativos mude entre as chamadas. Os argumentos antigos `app` e
`taskId` continuam aceitos durante a migração.

Hosts e gateways MCP não compartilham um único timeout longo de chamada de
ferramenta; alguns impõem um teto de cinco minutos e outros são
configuráveis. Não projete uma integração de agente externo em torno de um
`tools/call` bloqueante de dez minutos. `ask-agent` e `ask_app` confirmam o
trabalho hospedado rapidamente e depois retornam um `taskHandle` durável e um
`taskId` legado quando a espera embutida expira. Consulte com `ask_app_status`
reutilizando sem alterações os `poll.arguments` retornados, especialmente o
`taskHandle`; o resultado inclui `pollAfterMs` como um intervalo
amigável para hosts. A tarefa precisa ser criada antes que a primeira
...
com os mesmos `poll.arguments`, incluindo o `taskHandle` quando presente. Resultados
legados sem identificador devem reutilizar o mesmo `app` e `taskId`; nunca reenvie `ask_app` para recuperar,
...
<Endpoint id="doc-block-1chli4s" method="GET" path="/_agent-native/open" summary={"Rota aberta de link direto — concentra o navegador UI em um registro"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
A entrada executa `npx @agent-native/core@latest mcp serve --app <id>`, que por padrão é um **proxy stdio fino** para o `/mcp` do aplicativo local em execução — portanto, o registro de ação ao vivo, HMR, e os links diretos corretos permanecem a única fonte da verdade. Passe `--standalone` para construir o registro em processo. Quando `npx @agent-native/core@latest mcp install` detecta uma origem hospedada (um `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL` não localhost no espaço de trabalho `.env`), ele grava uma entrada de cliente `http` apontando para `<origin>/mcp` com um `Bearer` JWT em vez de uma entrada stdio.
...
- O endpoint `/mcp` existente aceita esse token como qualquer outro portador (consulte [MCP Protocol](/docs/mcp-protocol)) — sem novo endpoint, sem novo transporte.
...
- [WebMCP](/docs/webmcp) — ferramentas locais à página, o limite de 500.000 caracteres para entrada/resultado, e o cabeçalho de id da aba do navegador.
...
- [Actions](/docs/actions-overview) — definindo actions, `publicAgent`, GET / `readOnly`.
