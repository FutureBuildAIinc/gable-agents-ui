...
<WireframeBlock id="doc-block-mg0evs">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Engineering-Clips</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>OKR-Review</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Onboarding-Ablauf</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Bug-Repro</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>Agentenlesbar</span><span>Transkript + Frames bereit für Freigabelinks</span><div style='flex:1'></div><button>Teilen</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>Transkriptsuche</strong><div class='wf-box'>Treffer Kapitel 03:12 · Rollout-Risiken und Owner-Übergabe</div><div class='wf-box'>Meeting-Zusammenfassung und Aufgaben</div></div></div>"
    }
  />
</WireframeBlock>

Asynchrone Bildschirmaufzeichnungen, mit dem Kalender synchronisierte Besprechungsnotizen und Push-to-Talk-Diktate in einer App – der Agent ist auf jeder Oberfläche ein erstklassiger Editor, und die Aufzeichnungen, Besprechungen und Diktate gehören Ihnen, nicht einem SaaS-Anbieter. Clips macht freigegebene Aufzeichnungen außerdem für Agenten lesbar: Fügen Sie einen öffentlichen Clips-Freigabelink oder einen temporären **Für Agent kopieren**-Link in einen Agenten ein. Er kann das Transkript als Text „hören“ und mit Zeitstempeln versehene Bildschirmframes als Bilder „sehen“ – ganz ohne Rohvideo. Die Frame-Anzeige funktioniert in jedem bildfähigen Agenten (ChatGPT, Claude Code, Cursor, Codex); reine Text-Webchats erhalten weiterhin das vollständige Transkript und können einen von Ihnen hochgeladenen Frame verwenden.

<Diagram id="doc-block-1j41dwq" title="Erfassen, transkribieren, wiederverwenden" summary={"Drei Eroberungsarten landen in einer Bibliothek; Der Agent transkribiert, betitelt und fasst zusammen, dann ist jedes Transkript durchsuchbar und kann geteilt werden."}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">Bildschirmaufnahme</div>
    <div class="diagram-node">Kalendermeeting</div>
    <div class="diagram-node">Fn-halten-Diktat</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Eine Bibliothek<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">Titel · Zusammenfassung · Kapitel</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">Suchen</div>
    <div class="diagram-pill">Teilen</div>
    <div class="diagram-pill">Agentenlesbar links</div>
  </div>
</div>
```
...
- **Importieren Sie öffentliche Loom-Videos.** Wählen Sie im Web **Aufzeichnen → Loom importieren** und fügen Sie dann eine Loom-Freigabe- oder Einbettungs-URL ein. Clips speichert eine Kopie in Ihrer Bibliothek und importiert das öffentliche Transkript, sofern eines verfügbar ist.
...
- **Vorschau öffentlicher Clips in Slack** mit einer abspielbaren Video-Vorschau, nachdem
...
<Endpoint id="doc-block-115g6lb" title={"Einstiegspunkt für den Agentenkontext"} method="GET" path="/api/agent-context.json" summary={"Kompakte, für Agenten lesbare Beschreibung eines freigegebenen Clips"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-gih1ny" title="Transkript mit Zeitstempel" method="GET" path="/api/agent-transcript.json" summary="Transkript mit Zeitstempel segments for a shared clip" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1nhcw6o" title="Frame mit Zeitstempel" method="GET" path="/api/agent-frame.jpg" summary="Ein JPEG-Frame, der mit einem Originalvideo-Zeitstempel aus dem Video extrahiert wurde" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
1. **Videospeicher (erforderlich).** Verbinden Sie ein Speicher-Backend über den Onboarding-Assistenten. Der einfachste Weg ist Builder.io (free tier includes video storage and AI credits, ein Klick). Für selbst gehosteten Speicher stellen Sie `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` und optional `S3_REGION` und `S3_PUBLIC_BASE_URL` ein. Cloudflare R2 und DigitalOcean Spaces verwenden dieselben Umgebungsvariablen mit dem Präfix `R2_*`.
...
2. **Konfigurieren Sie den Produktionsstatus.** Legen Sie einen dauerhaften `DATABASE_URL` fest, den Normalen
   ...
   Videospeicheranbieter. Builder.io Connect (free tier includes video storage and AI credits) ist der einfachste Speicherpfad; für
...
6. **Verbinden Sie optionale Integrationen.** Google Calendar unterstützt die Registerkarte „Besprechungen“,
   `GEMINI_API_KEY` oder Builder.io Connect (free tier includes AI credits) ermöglicht die Bereinigung von Transkripten und Titeln
...
<DataModel
  id="doc-block-1fvw4i0"
  title="Clips Kerndatenmodell"
  summary={
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          name: "edits_json",
          type: "text",
          note: "Zerstörungsfreie Bearbeitungen",
        },
        {
          name: "chapters_json",
          type: "text",
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Datenschutz: Passwort / Ablauf",
        },
      ],
    },
    {
      ...
      note: "Aufteilen, damit die Bibliotheks- und Transkriptansichten schnell gerendert werden",
      fields: [
        {
          name: "recording_id",
          type: "text",
...
Jeder vom Agenten aufrufbare Vorgang ist eine TypeScript-Datei in `templates/clips/actions/`, die automatisch bei `POST /_agent-native/actions/:name` gemountet wird und vom CLI als `pnpm action <name>` ausgeführt werden kann. Es gibt etwa 100 Aktionen; die nützlichen Gruppierungen:
