---
title: "Externe Agenten: Claude, ChatGPT, Codex, Cursor, Cowork, Grok"
description: "Verbinden Sie Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok oder einen beliebigen MCP-kompatiblen Host mit einer gehosteten agentennativen App – und leiten Sie Artefakte dann mit MCP Apps und Deep Links zurück in das laufende UI."
search: "Claude Anthropic ChatGPT OpenAI Claude Code Codex Cursor Claude Cowork Grok xAI MCP Apps Agent-nativ Verbindung lokaler Agent Tools externe Agenten"
---
...
<div class="xa-trip">
  ...
  <div class="diagram-box" data-rough>
    Writes <code>navigate</code> app-state<br /><small class="diagram-muted"
      >UI fokussiert das Artefakt</small
    >
  </div>
</div>
```
...
- **Ihre eigene Laufzeit hinter dem Agent-Native-Chat:** siehe [Agent Surfaces](/docs/agent-surfaces#byo-agent) und [Native Chat-Oberfläche](/docs/native-chat-ui#byo-agent-runtimes), wenn ein Agent, der mit einem anderen Framework erstellt wurde, `<AssistantChat runtime={...}>` unterstützen soll.
...
- **Lokale benutzerdefinierte Unteragenten:** verwenden Sie [Agent-Ressourcen](/docs/agent-resources), wenn Sie benutzerdefinierte Agentenprofile innerhalb des agentennativen Arbeitsbereichs selbst wünschen.

## Einfache Einrichtung {#easy-setup}

Jede Agent-Native-App stellt eine kanonische MCP-URL bereit:

```text
https://<your-app>.agent-native.com/mcp
```

Öffnen Sie in der App **Settings → Integrations** und suchen Sie nach dem Ihnen
bekannten Hostnamen: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT,
OpenAI, Codex, Grok oder xAI. Das einzige Ergebnis für externe Hosts öffnet
**Settings → MCP** und wählt die passendste Anleitung aus. Kopieren Sie die
**MCP server URL** und fügen Sie sie in den KI-Host ein. Die URL ist die
Basis-URL der bereitgestellten App plus `/mcp`.
...
hostspezifische Registerkarten für Claude, ChatGPT, Cursor, Claude Code, Codex, Grok und Andere.

<a id="access-tab"></a>

### Die In-App-MCP-Einstellung {#mcp-settings}

Die Standard-App-Einstellungen enthalten neben **Integrations** einen **MCP**-Tab.
Er zeigt die kanonische MCP-URL der App, bei Verfügbarkeit eine A2A-Agent-Card
und dieselben Einrichtungsanleitungen wie die vollständige Verbindungsseite.
Verwenden Sie **Settings → MCP** als Startpunkt in der App oder öffnen Sie
`/mcp/connect` direkt für den vollständigen Ablauf und den Token-Fallback.
...
ChatGPT: Benutzerdefinierte MCP-Apps benötigen einen unterstützten ChatGPT-Web-
Arbeitsbereich. Wenn Sie Zugriff haben, öffnen Sie **Settings → Apps → Advanced
settings**, aktivieren Sie bei Bedarf den Entwicklermodus, wählen Sie **Create
app**, fügen Sie dieselbe MCP-URL ein, wählen Sie OAuth, scannen Sie die Tools,
melden Sie sich mit Agent-Native an, genehmigen Sie die angeforderten Bereiche
und aktivieren Sie die App in einem Chat. Business-, Enterprise- und Edu-
Arbeitsbereiche unterstützen vollständiges MCP einschließlich Schreibaktionen;
Pro unterstützt Lesen und Abrufen im Entwicklermodus. Wenn **Apps** oder
**Create app** fehlt, erlaubt Ihr Tarif oder Ihre Workspace-Richtlinie diese
Einrichtung nicht.
...
`reconnect` findet jeden MCP-Konfigurationseintrag, dessen URL auf `/mcp` für den angegebenen Host und ausgewählten Client endet (Übereinstimmung mit URL unabhängig vom Connector-Namen), und aktualisiert oder ersetzt dann das Authentifizierungsmaterial, ohne Ihren installierten skills zu berühren oder den vollständigen Installationsablauf erneut auszuführen. Übergeben Sie die Basis-App URL (z. B. `https://plan.agent-native.com`) – das Suffix `/mcp` wird abgeleitet. Authentifizierung und Tool-Laden erfolgen pro Client. Starten Sie diesen Client daher anschließend neu bzw. laden Sie ihn neu. Codex benötigt eine neue Sitzung, bevor neu geladene Tools angezeigt werden.
...
Für MCP-Clients, die keinen Remote-OAuth URL direkt hinzufügen können, öffnen Sie die App in Ihrem Browser und nutzen Sie das **Connect**-Angebot (bereitgestellt unter `https://&lt;app&gt;/mcp/connect`). Klicken Sie im angemeldeten Zustand auf **Verbinden / Autorisieren**. Auf der Seite erhalten Sie entweder einen One-Click-Deep-Link, der einen erkannten Agenten konfiguriert, oder einen zum Einfügen bereiten `.mcp.json`-Block:
...
<Diagram id="doc-block-1kyg3i1" title="Zwei Katalogebenen" summary={"Jeder Anrufer erhält standardmäßig die Kompaktstufe; Die vollständige ~105-Tool-Oberfläche ist nur optional. Die Werkzeugsuche schließt die Lücke, sodass nichts wirklich verborgen bleibt."}>
...
Dies ist ein allgemeines Richtlinienbeispiel. Analytics setzt `authenticatedReads: "off"` absichtlich und stellt nur die geprüften `connectorCatalog`-Actions bereit.
...
Eine App, die `resolveExecutionContext` bereitstellt, ersetzt diese standardmäßige Identitätsprüfung ausdrücklich. Ihr Resolver muss nicht verifizierte Aufrufer, Gäste, externe Mitglieder und nicht verknüpfte Workspaces eigenständig ablehnen, bevor er einen Principal zurückgibt.

Eine verifizierte Slack-DM wird vor dem Start des Agenten einem bestehenden Mitglied der Agent-Native-Organisation zugeordnet und als Identitätsverknüpfung zwischen Workspace und Benutzer gespeichert. Der daraus entstehende Benutzer-/Organisationskontext lädt die Ressourcen, Anweisungen und Skills dieses Benutzers. Standardmäßig werden erfolgreich geladene Workspace-Mitglieder abgelehnt, wenn ihre E-Mail-Adresse fehlt oder sie noch kein Organisationsmitglied sind. Apps können ausdrücklich `allowAnonymousOrgScopedSlackDm: true` setzen, um diese Mitglieder stattdessen als anonymen, auf die Organisation begrenzten Service-Principal auszuführen — mit derselben organisationsweiten Sichtbarkeit wie gemeinsame Kanäle, aber niemals mit benutzerprivatem Zugriff. Der Agent erhält dazu einen sichtbaren Hinweis, und Slack erklärt einmalig, wie persönlicher Zugriff erlangt werden kann. Dieses Opt-in lässt niemals fehlgeschlagene Profilladungen, nicht verifizierte Aufrufer, Gäste, externe Mitglieder (Slack Connect) oder Workspaces ohne Organisationsverknüpfung zu; diese werden höflich abgelehnt, statt die Nachricht stillschweigend zu verwerfen. Gemeinsame Slack-Kanäle verwenden einen Service-Principal und übernehmen keine privaten Berechtigungen eines Teilnehmers. Managed OAuth und das generierte Slack-App-Manifest fordern den Bot-Scope `users:read.email`. Bestehende Installationen müssen erneut verbunden/installiert werden, um einen neu hinzugefügten Scope zu gewähren; ältere Bot-Token-Installationen müssen ihn in Slack manuell hinzufügen. Ohne diesen Scope werden DMs abgelehnt, sofern die App `allowAnonymousOrgScopedSlackDm` nicht ausdrücklich aktiviert.
...
Externe Agenten erhalten die echten Erstellungs-/Aktualisierungswerkzeuge der App – dieselben benannten Aktionen, die auch der In-App-Agent aufruft – sodass sie, wenn ein Nutzer neuen Inhalt anfordert (ein Design, eine Präsentation, ein Formular, ein Dokument), diesen direkt verfassen und mit diesen Werkzeugen speichern sollten, statt die Arbeit über `ask_app` zu delegieren. Aktionen, die mit `endsTurn: true` markiert sind – etwa ein In-App-Fragenformular, das auf die Antwort des Nutzers im Chat dieser App wartet – sind vor MCP, WebMCP und A2A verborgen, sofern nicht `mcpTool: true` gesetzt ist – siehe [endsTurn in Actions definieren](/docs/actions-defining). Tool-Eingaben und -Ergebnisse tragen auf WebMCP bis zu 500.000 Zeichen, sodass ein ganzer Bildschirm, eine Präsentation oder ein Dokument in einem einzigen Schreibvorgang Platz findet – siehe [WebMCP](/docs/webmcp). Seiten-lokale WebMCP-Aufrufe tragen die Browser-Tab-ID (denselben `getBrowserTabId()`-Wert, den auch das UI verwendet – siehe [Kontextbewusstsein](/docs/context-awareness)), sodass sich tab-bezogene Navigation und Auswahl auf genau den Tab auflösen, den der Agent gerade steuert, selbst wenn er verborgen ist.
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | Agentenschleife | leitet eine Aufgabe in natürlicher Sprache an den In-App-Agenten dieser App weiter; bei länger laufender Arbeit werden ein dauerhaftes `taskHandle` und eine veraltete `taskId` für das Polling mit `ask_app_status` zurückgegeben |
...
### Lang laufende Agent-Anfragen

Wenn `ask_app` ein `taskHandle` zurückgibt, übergeben Sie diesen undurchsichtigen
Wert unverändert an `ask_app_status`. Er bewahrt die ursprüngliche Task-Route,
auch wenn sich die App-Erkennung zwischen Aufrufen ändert. Die älteren Argumente
`app` und `taskId` bleiben während der Migration gültig.
...
`ask_app_status` mit genau den zurückgegebenen `poll.arguments`, einschließlich
`taskHandle`, wenn vorhanden. Ältere Ergebnisse ohne Handle müssen dasselbe `app`
und dieselbe `taskId` wiederverwenden. Reichen Sie zur Wiederherstellung niemals erneut `ask_app` ein, da dies Arbeit duplizieren kann.
...
<Endpoint id="doc-block-1ur1b8j" method="GET" path="/_agent-native/open" summary={"Offene Deep-Link-Route: Fokussiert die Browser-Benutzeroberfläche auf einen Datensatz"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
`GET` + `readOnly` hält die Aktion ohne Nebenwirkungen und außerhalb des Bildschirmaktualisierungs-Änderungsereignisses. `publicAgent` ist das **explizite Opt-in** – eine öffentliche Webroute impliziert niemals eine öffentliche MCP/A2A-Exposition; siehe [Actions](/docs/actions-overview). Design/Inhaltsaufnahme actions MUST liest den **Live**-Status (das Yjs-Gemeinschaftsdokument, nicht die veraltete DB-Snapshot-Spalte), damit der externe Agent sieht, was der Benutzer tatsächlich auf dem Bildschirm hat. `pull-document` von Content leitet alle offenen Live-Zusammenarbeitssitzungen zuerst an SQL weiter. Der `get-design-snapshot` von design gibt den Inhalt der Live-Yjs-Datei sowie die vom Benutzer aufgelösten Optimierungswerte zurück.
...
- Der vorhandene `/mcp`-Endpunkt akzeptiert dieses Token wie jeder andere Träger (siehe [MCP Protocol](/docs/mcp-protocol)) – kein neuer Endpunkt, kein neuer Transport.
...
- [WebMCP](/docs/webmcp) – seitenlokale Tools, das 500.000-Zeichen-Limit für Eingabe/Ergebnis und der Browser-Tab-ID-Header.
...
- [Actions](/docs/actions-overview) – Definition von actions, `publicAgent`, GET / `readOnly`.
