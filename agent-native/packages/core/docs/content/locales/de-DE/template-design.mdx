...
<WireframeBlock id="doc-block-rsu1x4">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Produkt-Launch-Seite</h1><span class='wf-pill accent'>Desktop</span><span class='wf-pill'>Tablet</span><span class='wf-pill'>Mobile</span><div style='flex:1'></div><button>Preview</button><button class='primary'>Export code</button></div><div class='wf-card' style='flex:1;display:grid;grid-template-rows:auto 1fr auto;gap:12px'><div style='display:flex;gap:8px'><span class='wf-pill accent'>Hero</span><span class='wf-pill'>Pricing</span><span class='wf-pill'>FAQ</span></div><div class='wf-box' style='display:flex;flex-direction:column;gap:12px;min-height:230px;padding:20px'><div style='display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center'><strong style='font-size:22px'>Schneller starten mit Agent-Native</strong><span class='wf-muted' style='max-width:70%'>Erzeugen Sie aus einem Prompt einen funktionierenden Prototyp und verfeinern Sie ihn direkt.</span><button class='primary'>Loslegen</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:10px;flex:1'><div class='wf-card' style='min-height:56px'></div><div class='wf-card' style='min-height:56px'></div><div class='wf-card' style='min-height:56px'></div></div></div><div class='wf-card' style='display:flex;align-items:center;gap:10px'><span class='wf-muted'>Mach den Hero dichter und den CTA klarer.</span><div style='flex:1'></div><button class='primary'>Revision anwenden</button></div></div></div>"
    }
  />
</WireframeBlock>
...
Die folgende Tabelle behandelt die zentralen eigenen Ressourcen – Designs, Dateien, Vorlagen, Versionen und Designsysteme. Sieben weitere Tabellen im selben Schema unterstützen Funktionen, die an anderer Stelle in diesem Dokument beschrieben werden, und gehören nicht zum zentralen Ressourcenmodell: `design_localhost_connections` und `design_localhost_write_grants` (Localhost-Quellmodus), `component_index` (Komponentenindexierung), `motion_timeline` (Bewegung), `design_state` (Momentaufnahmen des Editorzustands), `design_fusion_edits` (Fusion-Modus für vollständige Apps) und `design_review_snapshot` (Review-Momentaufnahmen).
...
abgemeldete Benutzer gehen durch `/sign-in?return=...`;
