---
title: "外部代理：Claude、ChatGPT、Codex、光标、Cowork、Grok"
description: "将 Claude、ChatGPT、Codex、Cursor、Claude Cowork、Grok 或任何 MCP 兼容主机连接到托管代理本机应用程序，然后使用 MCP 应用程序和深层链接将往返工件返回到正在运行的 UI。"
search: "Claude Anthropic ChatGPT OpenAI Claude 代码 Codex 光标 Claude Cowork Grok xAI MCP 应用程序代理-本地连接本地代理工具外部代理"
---
...
<Diagram id="doc-block-p3if9r" title={"外部代理往返"} summary={"外部主机通过 MCP 调用工具；该应用程序返回一个工件和一个打开链接。单击它可以解析浏览器会话并将工件聚焦在正在运行的 UI 中 - 该链接不带有特权状态。"}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    外部宿主<br /><small class="diagram-muted"
...
- **Agent-Native 聊天背后您自己的运行时：**当使用另一个框架构建的代理应该为 `<AssistantChat runtime={...}>` 提供支持时，请参阅 [Agent Surfaces](/docs/agent-surfaces#byo-agent) 和 [Native 聊天界面](/docs/native-chat-ui#byo-agent-runtimes)。
...
- **本地自定义子代理：**当您希望在代理本机工作区本身内部自定义代理配置文件时，请使用 [代理资源](/docs/agent-resources)。

## 轻松设置 {#easy-setup}

每个 Agent-Native 应用都提供一个规范的 MCP URL：

```text
https://<your-app>.agent-native.com/mcp
```

在应用中打开 **Settings → Integrations**，然后搜索您熟悉的主机名称：
Claude、Claude Cowork、Claude Code、Anthropic、ChatGPT、OpenAI、Codex、Grok
或 xAI。唯一的外部主机结果会打开 **Settings → MCP**，并选中最匹配的指南。
复制 **MCP server URL** 并粘贴到 AI 主机中。URL 是已部署应用的基础 URL 加上
`/mcp`。
...
每个托管应用程序还有一个帮助页面
`https://&lt;app&gt;/mcp/connect` 与可复制的 URL 和
Claude、ChatGPT、光标、Claude 代码、Codex、Grok 和其他的主机特定选项卡。

<a id="access-tab"></a>

### 应用内的 MCP 设置 {#mcp-settings}

标准应用 Settings 在 **Integrations** 旁边提供 **MCP** 选项卡。它显示应用的规范
MCP URL、可用时的 A2A 代理卡片，以及与完整连接页面相同的设置指南。使用
**Settings → MCP** 作为应用内起点，或直接打开 `/mcp/connect` 查看完整流程和令牌回退。
...
ChatGPT：自定义 MCP 应用需要受支持的 ChatGPT web 工作区。如果你有访问权限，请打开
**Settings → Apps → Advanced settings**，在需要时启用开发者模式，选择 **Create app**，
粘贴相同的 MCP URL，选择 OAuth，扫描工具，使用 Agent-Native 登录，批准请求的范围，
然后在聊天中启用该应用。Business、Enterprise 和 Edu 工作区支持完整 MCP，包括写入
操作；Pro 在开发者模式下支持读取和获取。如果没有 **Apps** 或 **Create app**，则你的
套餐或工作区策略不允许此设置。
...
快照。`mcp:read` / `mcp:write` / `mcp:apps` / `offline_access` 范围与下文
[标准远程 MCP OAuth](#standard-oauth) 中介绍的相同。
...
`reconnect` 查找给定主机和选定客户端的 URL 以 `/mcp` 结尾的任何 MCP 配置条目（通过 URL 进行匹配，无论连接器名称如何），然后刷新或替换身份验证材料，而无需触及已安装的 skills 或重新运行完整安装流程。传递基本应用程序 URL（例如 `https://plan.agent-native.com`） - 推断出 `/mcp` 后缀。身份验证和工具加载是针对每个客户端的，因此之后重新启动/重新加载该客户端； Codex 在新加载的工具出现之前需要一个新会话。
...
对于无法直接添加远程 OAuth URL 的 MCP 客户端，请在浏览器中打开应用程序并使用其 **Connect** 功能（在 `https://&lt;app&gt;/mcp/connect` 上提供）。登录后，单击“**连接/授权**”。该页面为您提供一个配置检测到的代理的一键深层链接，或一个可立即粘贴的 `.mcp.json` 块：
...
| `offline_access` | 刷新令牌访问权限，使主机保持连接                 |

当客户端请求没有明确的范围时，应用程序会授予所有四个范围，因此连接器的行为类似于浏览器授权的连接流，并且可以在无需再次登录的情况下刷新访问权限。为本地开发人员、后备主机和需要准备粘贴配置块的客户端保留不记名令牌连接页面和 `npx @agent-native/core@latest connect --token <token>` 后备。
...
<Diagram id="doc-block-172xyxa" title={"两个目录层"} summary={"默认情况下，每个调用者都会获得紧凑层；完整的约 105 个工具表面仅供选择加入。工具搜索弥补了这一差距，因此没有什么是真正隐藏的。"}>
...
这是一个通用政策示例。Analytics 会有意设置 `authenticatedReads: "off"`，并且只公开经过审核的 `connectorCatalog` action。
...
提供 `resolveExecutionContext` 的应用会明确替换此默认身份判定流程。其解析器必须在返回主体前自行拒绝未经验证的调用者、访客、外部成员以及未关联的工作区。

经过验证的 Slack 私信会在代理运行前匹配现有的 Agent-Native 组织成员，并保存工作区/用户身份关联；由此生成的用户/组织上下文会加载该用户的资源、指令和技能。默认情况下，已成功获取资料但缺少电子邮件或尚未成为组织成员的工作区成员会被拒绝。应用可以明确设置 `allowAnonymousOrgScopedSlackDm: true`，让这些成员改用组织范围的匿名服务主体运行——其组织级可见性与共享频道相同，但绝不包含用户私有访问——同时向代理显示说明，并通过 Slack 一次性提示如何获得个人访问权限。此选择加入绝不会允许资料获取失败、未经验证的调用者、访客、外部（Slack Connect）成员或尚未连接组织的工作区；这些情况会收到礼貌的拒绝回复。Slack 共享频道使用服务主体，不会借用任何参与者的私人权限。托管 OAuth 和生成的 Slack 应用 manifest 都会请求 bot scope `users:read.email`。已有安装必须重新连接/安装才能授予新增的 scope；旧版 bot token 安装则必须在 Slack 中手动添加该 scope。没有此 scope 时，私信会被拒绝，除非应用明确启用 `allowAnonymousOrgScopedSlackDm`。
...
外部代理可以使用应用真正的创建/更新工具——与应用内代理调用的是同一批命名操作——因此当用户要求新内容(设计、幻灯片、表单、文档)时,应直接撰写并用这些工具保存,而不是把工作委托给 `ask_app`。标记为 `endsTurn: true` 的操作(例如在该应用自身聊天中等待用户回答的应用内提问表单),除非设置 `mcpTool: true`,否则会对 MCP、WebMCP 和 A2A 隐藏——见[定义 Actions 中的 endsTurn](/docs/actions-defining)。工具的输入和结果在 WebMCP 上最多可携带 500,000 个字符,因此整个界面、幻灯片或文档可以一次写入——见[WebMCP](/docs/webmcp)。页面本地的 WebMCP 调用还会携带浏览器标签页 ID(与 UI 使用的是同一个 `getBrowserTabId()` 值——见[情境意识](/docs/context-awareness)),因此按标签页划分的导航与选择会解析到代理正在操作的那个标签页,即便它被隐藏。
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | 代理循环 | 将自然语言任务路由到该应用的应用内代理；长时间运行的任务会返回一个持久的 `taskHandle` 和旧版 `taskId`，供 `ask_app_status` 轮询 |
...
当 `ask_app` 返回 `taskHandle` 时，请将这个不透明值原样传给
`ask_app_status`；即使两次调用之间的应用发现结果发生变化，它也会保留原始任务路由。
迁移期间仍接受旧的 `app` 和 `taskId` 参数。

MCP 主机和网关并不共享同一个较长的工具调用超时时间；有些强制执行五分钟的上限，有些则可配置。不要围绕一个阻塞十分钟的 `tools/call` 来设计外部代理集成。`ask-agent` 和 `ask_app` 会快速确认托管工作已开始，然后在内联等待到期时返回一个持久的 `taskHandle` 和旧版 `taskId`。请原样重用返回的 `poll.arguments`（尤其是 `taskHandle`）通过 `ask_app_status` 进行轮询；结果中包含 `pollAfterMs`，作为对主机友好的轮询间隔。任务必须在第一次响应超时之前就已创建，因此仅在长时间调用之后才返回的轮询 URL 并不是可靠的恢复约定。
...
持久任务失败。请使用完全相同的返回 `poll.arguments`（存在时包括 `taskHandle`）重试
`ask_app_status`；没有句柄的旧版结果必须重用相同的 `app` 和 `taskId`。切勿为恢复而
...
<Endpoint id="doc-block-6s47f" method="GET" path="/_agent-native/open" summary={"深层链接开放路线 — 将浏览器 UI 聚焦在记录上"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
`GET` + `readOnly` 保持操作无副作用，并且不受屏幕刷新更改事件的影响。 `publicAgent` 是**明确的选择加入** - 公共网络路由绝不意味着公开 MCP/A2A 暴露；见[Actions](/docs/actions-overview)。设计/内容摄取 actions MUST 读取**实时**状态（Yjs 协作文档，而不是过时的数据库快照列），以便外部代理看到用户在屏幕上实际显示的内容。内容的 `pull-document` 首先将任何开放的实时协作会话刷新到 SQL； design 的 `get-design-snapshot` 返回实时 Yjs 文件内容以及用户解析的调整值。
...
该条目运行 `npx @agent-native/core@latest mcp serve --app <id>`，默认情况下，它是正在运行的本地应用程序的 `/mcp` 的 **瘦 stdio 代理** - 因此，实时操作注册表、HMR 和正确的深层链接仍然是单一事实来源。通过 `--standalone` 来在进程中构建注册表。当 `npx @agent-native/core@latest mcp install` 检测到托管源（工作区 `.env` 中的非本地主机 `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL`）时，它会写入指向 `<origin>/mcp` 的 `http` 客户端条目，并使用 `Bearer` JWT 而不是 stdio 条目。
...
- 现有的 `/mcp` 端点像任何其他承载一样接受该令牌（请参阅 [MCP Protocol](/docs/mcp-protocol)） - 没有新端点，没有新传输。
...
- [WebMCP](/docs/webmcp) — 页面本地工具、500,000 字符的输入/结果上限,以及浏览器标签页 ID 请求头。
...
- [Actions](/docs/actions-overview) — 定义 actions、`publicAgent`、GET / `readOnly`。
