...
<WireframeBlock id="doc-block-nvqq22">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>工程 Clips</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>OKR 评审</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>入门流程</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Bug 复现</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>代理可读</span><span>转写 + 帧已可用于分享链接</span><div style='flex:1'></div><button>分享</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>转录搜索</strong><div class='wf-box'>匹配章节 03:12 · 发布风险和负责人交接</div><div class='wf-box'>会议摘要和行动项</div></div></div>"
    }
  />
</WireframeBlock>

异步屏幕录制、与日历同步的会议笔记和按键说话语音听写整合在一个应用中 — 代理是每个界面上的一流编辑器，而录音、会议和听写都属于您，而不是 SaaS 供应商。Clips 还让共享录音可供代理读取：将公开 Clips 分享链接或临时的**复制给代理**链接粘贴到代理中，它无需原始视频即可把转录文本作为文字“听到”，并把带时间戳的屏幕帧作为图像“看到”。帧查看适用于任何支持图像的代理（ChatGPT、Claude Code、Cursor、Codex）；纯文本网络聊天仍可获得完整转录文本，并可使用您上传的帧。

<Diagram id="doc-block-pkjyh1" title={"捕获、转录、重用"} summary={"三种捕获类型集中在一个库中；代理进行抄写、标题和总结，然后每个抄本都可以搜索和共享。"}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">屏幕录制</div>
    <div class="diagram-node">日历会议</div>
    <div class="diagram-node">按住 Fn 听写</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    一个库<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">标题 · 摘要 · 章节</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">搜索</div>
    <div class="diagram-pill">分享</div>
    <div class="diagram-pill">代理可读 links</div>
  </div>
</div>
```
...
- **导入公开的 Loom 视频。** 在网页端选择 **录制 → 导入 Loom**，然后粘贴 Loom 分享或嵌入 URL。Clips 会将副本保存到你的资料库，并在可用时导入公开的转录文本。
...
- **在 Slack 中预览公共剪辑**，工作区安装 Clips Slack 应用后会显示可播放的视频展开预览。
...
<Endpoint id="doc-block-1ntk9tk" title={"代理上下文入口点"} method="GET" path="/api/agent-context.json" summary={"共享剪辑的紧凑、代理可读的描述"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1e3o204" title={"带时间戳的文字记录"} method="GET" path="/api/agent-transcript.json" summary={"共享剪辑的带时间戳的转录片段"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-y18kg5" title={"具有时间戳的帧"} method="GET" path="/api/agent-frame.jpg" summary={"在原始视频时间戳处从视频中提取的 JPEG 帧"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
1. **视频存储（必需）。** 通过入门向导连接存储后端。最简单的路径是Builder.io（free tier includes video storage and AI credits，一键式）。对于自托管存储，请设置 `S3_ENDPOINT`、`S3_BUCKET`、`S3_ACCESS_KEY_ID`、`S3_SECRET_ACCESS_KEY` 以及可选的 `S3_REGION` 和 `S3_PUBLIC_BASE_URL`。 Cloudflare R2 和 DigitalOcean Spaces 使用带有 `R2_*` 前缀的相同环境变量。
...
2. **配置生产状态。**设置一个持久化的`DATABASE_URL`，正常
   ...
   视频存储提供商。 Builder.io Connect（free tier includes video storage and AI credits）是最简单的存储路径；对于
...
6. **连接可选集成。** Google Calendar 为“会议”选项卡提供支持，
   `GEMINI_API_KEY` 或 Builder.io Connect（free tier includes AI credits）支持转录清理和标题，
...
每个代理可调用操作都是 `templates/clips/actions/` 中的 TypeScript 文件，自动安装在 `POST /_agent-native/actions/:name` 上，并可作为 `pnpm action <name>` 从 CLI 运行。大约有 100 个 actions；有用的分组：
