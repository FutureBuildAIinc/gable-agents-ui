...
<WireframeBlock id="doc-block-1hz53a4">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-v2vwut" title={"提示到甲板"} summary={"请求一副牌，代理会通过您可以从 CLI 调用的相同操作一次滑入一张幻灯片。"}>
...
创建、编辑和删除演示文稿及幻灯片应使用共享 actions，例如 `create-deck`、`add-slide`、`update-slide` 和 `delete-deck`；代理和 UI 都可以调用它们。`templates/slides/server/routes/api/` 下的 REST 路由仅用于上传、流式传输、导出及其他路由专属流程。
