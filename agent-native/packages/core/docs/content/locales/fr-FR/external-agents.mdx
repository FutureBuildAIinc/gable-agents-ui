---
title: "Agents externes : Claude, ChatGPT, Codex, Curseur, Cowork, Grok"
description: "Connectez Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok ou tout hôte compatible MCP à une application native d'agent hébergée, puis transférez les artefacts aller-retour dans le UI en cours d'exécution avec les applications MCP et les liens profonds."
search: "Claude Anthropic ChatGPT OpenAI Claude Code Codex Curseur Claude Cowork Grok xAI MCP Applications agent natif connecter outils d'agent local agents externes"
---
...
<Diagram id="doc-block-4ree95" title={"L’aller-retour des agents externes"} summary={"Un hôte externe appelle un outil via MCP ; l'application renvoie un artefact plus un lien Ouvrir. En cliquant dessus, vous résolvez la session du navigateur et concentrez l'artefact dans l'interface utilisateur en cours d'exécution : le lien ne comporte aucun état privilégié."}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    Hôte externe<br /><small class="diagram-muted"
      >Claude &middot; ChatGPT &middot; Codex &middot; Cursor</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Outil MCP call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    L’app produit un artefact<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>L’utilisateur clique sur le lien</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
  ...
  <div class="diagram-box" data-rough>
    Writes <code>navigate</code> app-state<br /><small class="diagram-muted"
      >L’UI focalise l’artefact</small
    >
  </div>
</div>
```
...
- **Votre propre environnement d'exécution derrière le chat Agent-Native :** consultez [Agent Surfaces](/docs/agent-surfaces#byo-agent) et [Native Interface de chat](/docs/native-chat-ui#byo-agent-runtimes) lorsqu'un agent créé avec un autre framework devrait alimenter `<AssistantChat runtime={...}>`.
...
- **Sous-agents personnalisés locaux :** utilisez [Ressources de l'agent](/docs/agent-resources) lorsque vous souhaitez des profils d'agent personnalisés dans l'espace de travail natif de l'agent lui-même.

## Configuration facile {#easy-setup}

Chaque application Agent-Native expose une URL MCP canonique :

```text
https://<your-app>.agent-native.com/mcp
```

Dans l'application, ouvrez **Settings → Integrations** et recherchez le nom
d'hôte que vous connaissez déjà : Claude, Claude Cowork, Claude Code,
Anthropic, ChatGPT, OpenAI, Codex, Grok ou xAI. L'unique résultat d'hôte
externe ouvre **Settings → MCP** avec le guide le plus adapté déjà sélectionné.
Copiez la **MCP server URL** et collez-la dans l'hôte d'IA. L'URL correspond à
l'URL de base de l'application déployée suivie de `/mcp`.
...
onglets spécifiques à l'hôte pour Claude, ChatGPT, Cursor, Claude Code, Codex, Grok et autres.

<a id="access-tab"></a>

### Le réglage MCP dans l'application {#mcp-settings}

Les réglages standard de l'application incluent un onglet **MCP** à côté de
**Integrations**. Il affiche l'URL MCP canonique de l'application, une fiche
d'agent A2A si elle est disponible et les mêmes guides que la page de connexion
complète. Utilisez **Settings → MCP** comme point de départ ou ouvrez
`/mcp/connect` directement pour le flux complet et le repli par jeton.
...
ChatGPT : les applications MCP personnalisées nécessitent un espace de travail
Web ChatGPT compatible. Si vous y avez accès, ouvrez **Settings → Apps →
Advanced settings**, activez le mode développeur si nécessaire, choisissez
**Create app**, collez la même URL MCP, choisissez OAuth, analysez les outils,
connectez-vous avec Agent-Native, approuvez les portées demandées et activez
l'application dans une discussion. Les espaces Business, Enterprise et Edu
prennent en charge MCP complet, y compris les actions d'écriture ; Pro prend en
charge la lecture et la récupération en mode développeur. Si **Apps** ou
**Create app** n'apparaît pas, votre offre ou la politique de votre espace de
travail n'autorise pas cette configuration.
...
`reconnect` trouve toute entrée de configuration MCP dont URL se termine par `/mcp` pour l'hôte donné et le client sélectionné (correspondant à URL quel que soit le nom du connecteur), puis actualise ou remplace le matériel d'authentification sans toucher à votre skills installé ni réexécuter le flux d'installation complet. Transmettez l'application de base URL (par exemple `https://plan.agent-native.com`) — le suffixe `/mcp` est déduit. L'authentification et le chargement des outils se font par client, alors redémarrez/rechargez ce client par la suite ; Codex a besoin d'une nouvelle session avant que les outils nouvellement chargés n'apparaissent.
...
Pour les clients MCP qui ne peuvent pas ajouter directement un OAuth distant URL, ouvrez l'application dans votre navigateur et utilisez son offre **Connect** (servie sur `https://&lt;app&gt;/mcp/connect`). Une fois connecté, cliquez sur **Connecter/Autoriser**. La page vous remet soit un lien profond en un clic qui configure un agent détecté, soit un bloc `.mcp.json` prêt à coller :
...
<Diagram id="doc-block-13pys8t" title="Deux niveaux de catalogue" summary={"Chaque appelant obtient le niveau compact par défaut ; la surface complète d'environ 105 outils est uniquement facultative. la recherche par outils comble le fossé afin que rien ne soit jamais vraiment caché."}>
...
Il s'agit d'un exemple de politique générique. Analytics définit intentionnellement `authenticatedReads: "off"` et n'expose que les actions `connectorCatalog` qui ont été examinées.
...
Une application qui fournit `resolveExecutionContext` remplace explicitement cette chaîne d'identité par défaut. Son résolveur doit refuser indépendamment les appelants non vérifiés, les invités, les membres externes et les espaces de travail non liés avant de renvoyer un principal.

Avant l'exécution de l'agent, un message direct Slack vérifié est associé à un membre existant de l'organisation Agent-Native et le lien d'identité entre l'espace de travail et l'utilisateur est enregistré. Le contexte utilisateur/organisation qui en résulte charge les ressources, les instructions et les compétences de cet utilisateur. Par défaut, les membres de l'espace de travail dont le profil a bien été chargé, mais dont l'adresse e-mail est absente ou qui ne sont pas encore membres de l'organisation, sont refusés. Les applications peuvent définir explicitement `allowAnonymousOrgScopedSlackDm: true` pour exécuter ces membres comme principal de service anonyme limité à l'organisation : le même niveau de visibilité à l'échelle de l'organisation que celui des canaux partagés, sans jamais accéder aux données privées de l'utilisateur. L'agent reçoit une note visible et Slack explique une fois comment obtenir un accès personnel. Cette option n'admet jamais les échecs de chargement du profil, les appelants non vérifiés, les invités, les membres externes (Slack Connect) ni les espaces de travail non liés à une organisation ; ces cas reçoivent une réponse de refus polie. Les canaux Slack partagés utilisent un principal de service et n'empruntent pas les autorisations privées d'un participant. OAuth géré et le manifeste d'application Slack généré demandent la portée de bot `users:read.email`. Les installations existantes doivent être reconnectées ou réinstallées pour accorder une portée nouvellement ajoutée ; les anciennes installations par jeton de bot doivent l'ajouter manuellement dans Slack. Sans cette portée, les messages directs sont refusés sauf si l'application active explicitement `allowAnonymousOrgScopedSlackDm`.
...
Les agents externes reçoivent les vrais outils de création/mise à jour de l'application — les mêmes actions nommées que l'agent intégré appelle — donc quand un utilisateur demande un nouveau contenu (un design, une présentation, un formulaire, un document), créez-le directement et enregistrez-le avec ces outils au lieu de déléguer le travail via `ask_app`. Les actions marquées `endsTurn: true`, comme un formulaire de questions intégré qui attend la réponse de l'utilisateur dans le chat de cette application, sont masquées de MCP, WebMCP et A2A sauf si `mcpTool: true` — voir [endsTurn dans Définir des actions](/docs/actions-defining). Les entrées et résultats d'outils supportent jusqu'à 500 000 caractères sur WebMCP, afin qu'un écran, une présentation ou un document entier tienne en une seule écriture — voir [WebMCP](/docs/webmcp). Les appels WebMCP locaux à la page transmettent l'identifiant de l'onglet du navigateur (la même valeur `getBrowserTabId()` que celle utilisée par l'UI — voir [Conscience du contexte](/docs/context-awareness)), afin que la navigation et la sélection propres à l'onglet se résolvent vers l'onglet que l'agent pilote, même lorsqu'il est masqué.
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | boucle d'agent     | achemine une tâche en langage naturel vers l'agent intégré à l'application ; les tâches longues renvoient un `taskHandle` durable et un `taskId` hérité pour l'interrogation via `ask_app_status` |
...
### Requêtes d'agent de longue durée

Lorsque `ask_app` renvoie un `taskHandle`, transmettez cette valeur opaque sans
la modifier à `ask_app_status` ; elle conserve la route d'origine de la tâche
même si la découverte des applications change entre deux appels. Les anciens
arguments `app` et `taskId` restent acceptés pendant la migration.
...
durable a échoué. Réessayez `ask_app_status` avec exactement les mêmes
`poll.arguments`, y compris le `taskHandle` lorsqu'il est présent. Les anciens
résultats sans handle doivent réutiliser les mêmes `app` et `taskId` ; ne soumettez jamais à nouveau `ask_app` pour récupérer, car la
...
<Endpoint id="doc-block-wpvcbp" method="GET" path="/_agent-native/open" summary={"Route ouverte par lien profond : concentre l'interface utilisateur du navigateur sur un enregistrement"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
L'entrée exécute `npx @agent-native/core@latest mcp serve --app <id>`, qui est par défaut un **thin stdio proxy** pour le `/mcp` de l'application locale en cours d'exécution — de sorte que le registre d'action en direct, HMR et les liens profonds corrects restent la source unique de vérité. Transmettez `--standalone` pour créer le registre en cours de processus. Lorsque `npx @agent-native/core@latest mcp install` détecte une origine hébergée (un hôte non local `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL` dans l'espace de travail `.env`), il écrit une entrée client `http` pointant vers `<origin>/mcp` avec un `Bearer` JWT au lieu d'une entrée stdio.
...
- Le point de terminaison `/mcp` existant accepte ce jeton comme n'importe quel autre support (voir [MCP Protocol](/docs/mcp-protocol)) — pas de nouveau point de terminaison, pas de nouveau transport.
...
- [WebMCP](/docs/webmcp) : outils locaux à la page, la limite de 500 000 caractères pour l'entrée/le résultat, et l'en-tête d'identifiant d'onglet du navigateur.
...
- [Actions](/docs/actions-overview) — définissant actions, `publicAgent`, GET / `readOnly`.
