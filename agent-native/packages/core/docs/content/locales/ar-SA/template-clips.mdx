...
<WireframeBlock id="doc-block-1f2wfe4">
  <Screen
    surface="desktop"
    html={
...
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">تسجيل الشاشة</div>
    <div class="diagram-node">اجتماع التقويم</div>
    <div class="diagram-node">Fn مع الاستمرار في الإملاء</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    مكتبة واحدة<br /><small class="diagram-muted"
      >التسجيلات + النصوص (SQL)</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Agent</span
    ><small class="diagram-muted">العنوان · الملخص · الفصول</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">يبحث</div>
    <div class="diagram-pill">يشارك</div>
    <div class="diagram-pill">روابط قابلة للقراءة من قبل الوكيل</div>
  </div>
</div>
```
...
- **استورد مقاطع فيديو Loom العامة.** على الويب، اختر **تسجيل ← استيراد Loom**، ثم الصق رابط مشاركة أو تضمين من Loom. يحفظ Clips نسخة في مكتبتك ويستورد النص العام عندما يكون متاحًا.
...
<Endpoint id="doc-block-34bupt" title={"نقطة إدخال سياق الوكيل"} method="GET" path="/api/agent-context.json" summary={"وصف مضغوط وسهل القراءة للمقطع المشترك"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1bv1x5j" title={"نص ذو طابع زمني"} method="GET" path="/api/agent-transcript.json" summary={"مقاطع نصية ذات طابع زمني لمقطع مشترك"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-1rwcmzz" title={"إطار في طابع زمني"} method="GET" path="/api/agent-frame.jpg" summary={"إطار JPEG مستخرج من الفيديو بطابع زمني للفيديو الأصلي"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
2. **تكوين حالة الإنتاج.** تعيين `DATABASE_URL` المستمر، العادي
   ...
   موفر تخزين الفيديو. Builder.io Connect (free tier includes video storage and AI credits) هو أسهل مسار للتخزين؛ ل
...
6. **الاتصال بعمليات التكامل الاختيارية.** تعمل Google Calendar على تشغيل علامة التبويب "الاجتماعات"،
   يعمل `GEMINI_API_KEY` أو Builder.io Connect (free tier includes AI credits) على تشغيل تنظيف النص والعناوين،
...
<DataModel
  id="doc-block-lu79or"
  title={"نموذج البيانات الأساسية للمقاطع"}
  summary={
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "الخصوصية: كلمة المرور / انتهاء الصلاحية",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          name: "source",
          type: "text",
          note: "فريق الجبهة الوطنية، وما إلى ذلك.",
        },
        {
          name: "target_app",
          type: "text",
