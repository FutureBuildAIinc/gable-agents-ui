---
title: "외부 에이전트: Claude, ChatGPT, Codex, 커서, Cowork, Grok"
description: "Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok 또는 MCP 호환 호스트를 호스팅된 에이전트 기본 앱에 연결한 다음 MCP 앱 및 딥 링크를 사용하여 아티팩트를 실행 중인 UI로 다시 왕복합니다."
search: "Claude Anthropic ChatGPT OpenAI Claude 코드 Codex 커서 Claude Cowork Grok xAI MCP 앱 에이전트 기본 연결 로컬 에이전트 도구 외부 에이전트"
---
...
<Diagram id="doc-block-18m3jj" title={"외부 에이전트 왕복"} summary={"외부 호스트가 MCP을 통해 도구를 호출합니다. 앱은 아티팩트와 Open 링크를 반환합니다. 이를 클릭하면 브라우저 세션이 해결되고 실행 중인 UI의 아티팩트에 초점이 맞춰집니다. 링크에는 권한 있는 상태가 없습니다."}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    외부 호스트<br /><small class="diagram-muted"
      >Claude &middot; ChatGPT &middot; Codex &middot; Cursor</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">MCP 도구 call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    앱이 산출물을 생성<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>사용자가 링크 클릭</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
...
- **Agent-Native 채팅 뒤에 숨은 자체 런타임:** 다른 프레임워크로 구축된 에이전트가 `<AssistantChat runtime={...}>`를 구동해야 하는 경우 [Agent Surfaces](/docs/agent-surfaces#byo-agent) 및 [Native 채팅 UI](/docs/native-chat-ui#byo-agent-runtimes)를 참조하세요.
...
- **로컬 사용자 지정 하위 에이전트:** 에이전트 기본 작업 영역 자체 내에서 사용자 지정 에이전트 프로필을 원할 경우 [에이전트 리소스](/docs/agent-resources)를 사용하세요.

## 간편한 설정 {#easy-setup}

모든 Agent-Native 앱은 표준 MCP URL을 제공합니다.

```text
https://<your-app>.agent-native.com/mcp
```

앱에서 **Settings → Integrations**를 열고 알고 있는 호스트 이름을 검색하세요.
Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI, Codex, Grok,
xAI 중 어떤 이름으로도 검색할 수 있습니다. 외부 호스트 결과 하나를 선택하면 가장 알맞은
가이드가 선택된 **Settings → MCP**가 열립니다. **MCP server URL**을 복사해 AI
호스트에 붙여넣으세요. URL은 배포된 앱의 기본 URL 뒤에 `/mcp`를 붙인 형식입니다.
...
복사 가능한 URL가 있는 `https://&lt;app&gt;/mcp/connect` 및
Claude, ChatGPT, 커서, Claude 코드, Codex, Grok 및 기타에 대한 호스트별 탭.

<a id="access-tab"></a>

### 앱 내 MCP 설정 {#mcp-settings}

표준 앱 Settings에는 **Integrations** 옆에 **MCP** 탭이 있습니다. 앱의 표준 MCP
URL, 가능한 경우 A2A 에이전트 카드, 전체 연결 페이지와 같은 설정 가이드를 보여줍니다.
앱에서 시작하려면 **Settings → MCP**를 사용하거나, 전체 흐름과 토큰 대체 경로를
보려면 `/mcp/connect`를 직접 여세요.
...
ChatGPT: 맞춤 MCP 앱에는 지원되는 ChatGPT web 작업 공간이 필요합니다. 액세스할 수
있다면 **Settings → Apps → Advanced settings**를 열고, 필요한 경우 developer mode를
활성화한 뒤 **Create app**을 선택하세요. 같은 MCP URL을 붙여넣고 OAuth를 선택하여
도구를 스캔한 다음 Agent-Native에 로그인하고 요청된 범위를 승인한 뒤 채팅에서 앱을
활성화하세요. Business, Enterprise, Edu 작업 공간은 쓰기 작업을 포함한 전체 MCP를
지원하며, Pro는 developer mode에서 읽기와 가져오기를 지원합니다. **Apps** 또는
**Create app**이 보이지 않으면 요금제나 작업 공간 정책에서 이 설정을 허용하지 않는
것입니다.
...
스냅샷. `mcp:read` / `mcp:write` / `mcp:apps` / `offline_access` 범위는 아래의
[표준 원격 MCP OAuth](#standard-oauth)에서 설명하는 범위와 같습니다.
...
`reconnect`는 지정된 호스트 및 선택한 클라이언트(커넥터 이름에 관계없이 URL와 일치)에 대해 URL가 `/mcp`로 끝나는 MCP 구성 항목을 찾은 다음 설치된 skills를 건드리거나 전체 설치 흐름을 다시 실행하지 않고 인증 자료를 새로 고치거나 교체합니다. 기본 앱 URL(예: `https://plan.agent-native.com`)를 전달하면 `/mcp` 접미사가 유추됩니다. 인증 및 도구 로드는 클라이언트별로 이루어지므로 나중에 해당 클라이언트를 다시 시작하거나 다시 로드하세요. 새로 로드된 도구가 나타나려면 Codex에 새 세션이 필요합니다.
...
원격 OAuth URL를 직접 추가할 수 없는 MCP 클라이언트의 경우 브라우저에서 앱을 열고 **Connect** 어포던스(`https://&lt;app&gt;/mcp/connect`에서 제공)를 사용하세요. 로그인한 상태에서 **연결 / 승인**을 클릭하세요. 페이지에서는 감지된 에이전트를 구성하는 원클릭 딥 링크 또는 바로 붙여넣을 수 있는 `.mcp.json` 블록을 제공합니다.
...
| `offline_access` | 호스트 연결을 유지하기 위한 새로 고침 토큰 액세스         |

클라이언트가 명시적인 범위를 요청하지 않으면 앱은 네 가지 범위를 모두 허용하므로 커넥터는 브라우저에서 승인된 Connect 흐름처럼 작동하고 다시 로그인하지 않고 액세스를 새로 고칠 수 있습니다. 바로 붙여넣을 수 있는 구성 블록이 필요한 로컬 개발, 대체 호스트 및 클라이언트에 대해 베어러 토큰 연결 페이지와 `npx @agent-native/core@latest connect --token <token>` 대체를 유지하세요.
...
<Diagram id="doc-block-1v0t9nn" title={"두 개의 카탈로그 계층"} summary={"모든 호출자는 기본적으로 컴팩트 계층을 얻습니다. 전체 ~105개 도구 표면은 선택적으로만 제공됩니다. 도구 검색은 격차를 해소하므로 실제로 숨겨진 것은 없습니다."}>
...
<p class="diagram-muted note">
  <code>tool-search</code>는 필요할 때 전체 계층 도구를 검색하지만, 명시적으로
  전체 계층을 선택하지 않는 한 실행에는 커넥터 카탈로그 또는 인증된 읽기 정책의
  허용이 필요합니다.
</p>
```
...
기본적으로 연결된 모든 에이전트는 작고 선별된 카탈로그를 봅니다(약 20~30개의 도구 대 전체 표면의 약 105개). 앱은 명시적인 `connectorCatalog`를 관리하거나 인증된 읽기 정책을 선택할 수 있습니다.
...
`tool-search`는 두 가지 방식으로 작동합니다. 도구 이름의 전체 메뉴와 한 줄 설명(저렴함, 스키마 없음)은 **쿼리 없이** 호출하고, 매개변수 요약이 포함된 순위 결과는 쿼리와 함께 호출합니다. 이는 압축된 클라이언트가 기능을 찾도록 돕습니다. 앱 에이전트의 더 폭넓은 추론이나 쓰기가 필요한 작업에는 `ask_app`을 사용하세요.
...
이는 일반적인 정책 예시입니다. Analytics는 의도적으로 `authenticatedReads: "off"`를 설정하고 검토된 `connectorCatalog` action만 노출합니다.
...
`resolveExecutionContext`를 제공하는 앱은 이 기본 ID 확인 절차를 명시적으로 대체합니다. 해당 resolver는 principal을 반환하기 전에 검증되지 않은 호출자, 게스트, 외부 구성원, 연결되지 않은 workspace를 독립적으로 거부해야 합니다.

검증된 Slack DM은 에이전트가 실행되기 전에 기존 Agent-Native 조직 구성원과 연결되고 workspace/user ID 링크로 저장됩니다. 그 결과 생성된 사용자/조직 컨텍스트에는 해당 사용자의 리소스, 지침 및 skills가 로드됩니다. 기본적으로 프로필을 정상적으로 가져왔지만 이메일이 없거나 아직 조직 구성원이 아닌 workspace 구성원은 거부됩니다. 앱은 `allowAnonymousOrgScopedSlackDm: true`를 명시적으로 설정해 이러한 구성원을 조직 범위의 익명 service principal로 실행할 수 있습니다. 이는 공유 채널과 동일한 조직 전체 공개 수준이지만 사용자 비공개 데이터에는 절대 접근하지 않습니다. 에이전트에 표시되는 메모와 개인 액세스를 얻는 방법을 설명하는 일회성 Slack 안내가 함께 제공됩니다. 이 opt-in은 프로필 가져오기 실패, 검증되지 않은 호출자, 게스트, 외부(Slack Connect) 구성원 또는 조직에 연결되지 않은 workspace를 허용하지 않으며, 이들에게는 정중한 거절 응답을 보냅니다. Slack 공유 채널은 service principal을 사용하며 참여자의 개인 권한을 빌리지 않습니다. 관리형 OAuth와 생성된 Slack 앱 매니페스트는 모두 bot scope `users:read.email`을 요청합니다. 새로 추가된 scope를 부여하려면 기존 설치를 다시 연결/설치해야 하며, 이전 bot-token 설치는 Slack에서 scope를 수동으로 추가해야 합니다. 이 scope가 없으면 앱이 `allowAnonymousOrgScopedSlackDm`을 명시적으로 활성화하지 않는 한 DM은 거부됩니다.
...
외부 에이전트는 앱 내 에이전트가 호출하는 것과 동일한 이름의 실제 생성/수정 도구에 접근할 수 있습니다. 따라서 사용자가 새로운 콘텐츠(디자인, 덱, 폼, 문서 등)를 요청하면, 작업을 `ask_app`으로 위임하지 말고 직접 작성한 뒤 해당 도구로 저장하세요. `endsTurn: true`로 표시된 액션 ― 예를 들어 사용자의 답변을 해당 앱 자체의 채팅에서 기다리는 앱 내 질문 폼 ― 은 `mcpTool: true`가 아닌 한 MCP, WebMCP, A2A에서 숨겨집니다 ― [Actions 정의하기의 endsTurn](/docs/actions-defining)을 참고하세요. 도구 입력과 결과는 WebMCP에서 최대 500,000자까지 지원하므로, 화면, 덱, 문서 전체가 한 번의 쓰기에 담깁니다 ― [WebMCP](/docs/webmcp)를 참고하세요. 페이지 로컬 WebMCP 호출은 브라우저 탭 ID(UI가 사용하는 것과 동일한 `getBrowserTabId()` 값 ― [상황 인식](/docs/context-awareness) 참고)도 함께 전달하므로, 탭별 내비게이션과 선택 항목은 숨겨져 있더라도 에이전트가 조작 중인 탭으로 해석됩니다.
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | 에이전트 루프 | 자연어 작업을 해당 앱의 인앱 에이전트로 라우팅합니다. 오래 걸리는 작업은 `ask_app_status` 폴링을 위한 내구성 있는 `taskHandle`과 기존 `taskId`를 반환합니다 |
...
앱 호스트의 도구 및 리소스 카탈로그는 기본적으로 컴팩트합니다. [카탈로그 계층](#catalog-tiers)을 참조하세요. `publicAgent.expose`는 컴팩트 카탈로그 외부의 안전한 읽기/수집 도구를 action 단위로 선택하는 설정입니다. 앱은 `externalAgents.authenticatedReads: "auto"`를 설정하여 수동 카탈로그 없이 인증된 읽기를 노출할 수 있습니다. `mcpApp.compactCatalog: true`는 채팅 호스트 검색에 반드시 표시되어야 하는 action의 드문 예외에만 설정하세요.
...
`ask_app`이 `taskHandle`을 반환하면 해당 불투명 값을 변경하지 말고
`ask_app_status`에 전달하세요. 호출 사이에 앱 검색 결과가 바뀌어도 원래 작업 경로가
유지됩니다. 마이그레이션 중에는 기존 `app` 및 `taskId` 인수도 계속 허용됩니다.

MCP 호스트와 게이트웨이는 하나의 긴 도구 호출 제한 시간을 공유하지 않습니다. 일부는
5분 상한을 강제하고 다른 일부는 설정 가능합니다. 10분간 차단되는 `tools/call`을
중심으로 외부 에이전트 통합을 설계하지 마세요. `ask-agent`와 `ask_app`은 호스팅된
작업의 접수를 빠르게 확인한 뒤, 인라인 대기 시간이 만료되면 내구성 있는 `taskHandle`과
기존 `taskId`를 반환합니다. 반환된 `poll.arguments`, 특히 `taskHandle`을 변경하지 않고
재사용해 `ask_app_status`로 폴링하세요. 결과에는
호스트 친화적인 간격인 `pollAfterMs`가 포함됩니다. 첫 번째 응답이 시간 초과되기
...
반환된 동일한 `poll.arguments`로 `ask_app_status`를 재시도하고, 있는 경우 `taskHandle`을
포함하세요. 핸들이 없는 기존 결과는 동일한 `app`과 `taskId`를 재사용해야 합니다. 제출하면 작업이 중복될 수 있으므로
...
<Endpoint id="doc-block-owf9c6" method="GET" path="/_agent-native/open" summary={"딥링크 공개 경로 — 레코드에 브라우저 UI 초점을 맞춥니다."} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
- 기존 `/mcp` 엔드포인트는 다른 전달자와 마찬가지로 해당 토큰을 허용합니다([MCP Protocol](/docs/mcp-protocol) 참조). 새 엔드포인트나 새 전송이 없습니다.
...
- [WebMCP](/docs/webmcp) — 페이지 로컬 도구, 입력/결과 500,000자 제한, 브라우저 탭 ID 헤더.
...
- [Actions](/docs/actions-overview) — actions, `publicAgent`, GET / `readOnly`를 정의합니다.
