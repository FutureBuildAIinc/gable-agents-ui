...
<WireframeBlock id="doc-block-1yqqeq1">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>엔지니어링 Clips</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>OKR 검토</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>온보딩 흐름</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Bug 재현</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>에이전트가 읽을 수 있음</span><span>전사 + 프레임 공유 링크 준비 완료</span><div style='flex:1'></div><button>공유</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>기록 검색</strong><div class='wf-box'>일치한 챕터 03:12 · 롤아웃 위험 및 담당자 인계</div><div class='wf-box'>회의 요약 및 작업 항목</div></div></div>"
    }
  />
</WireframeBlock>

비동기 화면 녹화, 캘린더와 동기화된 회의 메모, 푸시 투 토크 음성 받아쓰기를 하나의 앱에 담았습니다. 에이전트는 모든 화면에서 일급 편집자로 작동하며 녹화, 회의, 받아쓰기는 SaaS 공급업체가 아니라 사용자의 것입니다. Clips는 공유 녹화도 에이전트가 읽을 수 있게 합니다. 공개 Clips 공유 링크나 임시 **에이전트용으로 복사** 링크를 에이전트에 붙여넣으면 원본 동영상 없이도 대본을 텍스트로 "듣고" 타임스탬프가 있는 화면 프레임을 이미지로 "볼" 수 있습니다. 프레임 보기는 이미지 지원 에이전트(ChatGPT, Claude Code, Cursor, Codex)에서 작동합니다. 텍스트 전용 웹 채팅도 전체 대본을 받고 사용자가 업로드한 프레임을 활용할 수 있습니다.

<Diagram id="doc-block-wz0gsz" title={"캡처, 복사, 재사용"} summary={"세 가지 캡처 유형이 하나의 라이브러리에 포함됩니다. 상담원이 스크립트를 작성하고 제목을 지정하고 요약하면 모든 스크립트를 검색하고 공유할 수 있습니다."}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">화면 녹화</div>
    <div class="diagram-node">캘린더 회의</div>
    <div class="diagram-node">Fn 길게 누르기 받아쓰기</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    하나의 라이브러리<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">제목 · 요약 · 챕터</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">검색</div>
    <div class="diagram-pill">공유</div>
    <div class="diagram-pill">에이전트가 읽을 수 있음 links</div>
  </div>
</div>
```
...
- **Slack에서 공개 클립을 미리 보세요**. 워크스페이스가 Clips Slack 앱을 설치하면 재생 가능한 동영상 미리보기가 펼쳐집니다.
...
<Endpoint id="doc-block-d3mwi2" title={"에이전트 컨텍스트 진입점"} method="GET" path="/api/agent-context.json" summary={"에이전트가 읽을 수 있는 공유 클립에 대한 간결한 설명"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-13evgbg" title={"타임스탬프가 있는 성적표"} method="GET" path="/api/agent-transcript.json" summary={"공유 클립의 타임스탬프 기록 세그먼트"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-10cduf0" title={"타임스탬프의 프레임"} method="GET" path="/api/agent-frame.jpg" summary={"원본 비디오 타임스탬프의 비디오에서 추출된 JPEG 프레임"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
1. **비디오 저장소(필수).** 온보딩 마법사를 통해 저장소 백엔드를 연결합니다. 가장 쉬운 경로는 Builder.io(free tier includes video storage and AI credits, 원클릭)입니다. 자체 호스팅 스토리지의 경우 `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`를 설정하고 선택적으로 `S3_REGION` 및 `S3_PUBLIC_BASE_URL`를 설정합니다. Cloudflare R2와 DigitalOcean Spaces는 `R2_*` 접두사가 있는 동일한 환경 변수를 사용합니다.
...
2. **프로덕션 상태를 구성합니다.** 영구 `DATABASE_URL`, 일반 설정
   ...
   비디오 저장 제공업체. Builder.io Connect(free tier includes video storage and AI credits)는 가장 쉬운 저장 경로입니다. 를 위해
...
6. **선택적 통합을 연결하세요.** Google Calendar는 회의 탭을 강화합니다.
   `GEMINI_API_KEY` 또는 Builder.io Connect(free tier includes AI credits)는 성적표 정리 및 제목을 강화합니다.
...
<DataModel
  ...
  entities={[
    ...
    },
    {
      ...
      note: "라이브러리 및 스크립트 보기가 빠르게 렌더링되도록 분할",
      fields: [
        {
          name: "recording_id",
          type: "text",
...
모든 에이전트 호출 가능 작업은 `templates/clips/actions/`의 TypeScript 파일이며, `POST /_agent-native/actions/:name`에 자동으로 마운트되고 CLI에서 `pnpm action <name>`로 실행 가능합니다. 약 100개의 actions가 있습니다. 유용한 그룹화:
