...
<WireframeBlock id="doc-block-1swnyjw">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Clips de ingeniería</h1><span class='wf-pill accent'>Library</span><span class='wf-pill'>Meetings</span><span class='wf-pill'>Dictation</span><div style='flex:1'></div><button>Import</button><button class='primary'>Record</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:12px'><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Revisión de OKR</strong><small>35 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Flujo de incorporación</strong><small>12 min</small></div><div class='wf-card' style='height:120px;display:flex;flex-direction:column;justify-content:end'><strong>Reproducción de bug</strong><small>4 min</small></div></div><div class='wf-card' style='display:flex;gap:10px;align-items:center'><span class='wf-pill accent'>Legible por agentes</span><span>Transcripción + fotogramas listos para enlaces compartidos</span><div style='flex:1'></div><button>Compartir</button></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:8px'><strong>Búsqueda de transcripciones</strong><div class='wf-box'>Capítulo coincidente 03:12 · riesgos de despliegue y traspaso de propietario</div><div class='wf-box'>Resumen de la reunión y acciones</div></div></div>"
    }
  />
</WireframeBlock>

Grabaciones de pantalla asíncronas, notas de reuniones sincronizadas con el calendario y dictado por pulsación para hablar, todo en una sola aplicación: el agente es un editor de primera clase en cada superficie y las grabaciones, reuniones y dictados son suyos, no de un proveedor de SaaS. Clips también hace que las grabaciones compartidas sean legibles para los agentes: pegue un enlace público de Clips o un enlace temporal **Copiar para el agente** en un agente y podrá «oír» la transcripción como texto y «ver» como imágenes los fotogramas de pantalla con marca de tiempo, sin necesidad del vídeo original. La visualización de fotogramas funciona en cualquier agente con capacidad de imagen (ChatGPT, Claude Code, Cursor, Codex); los chats web de solo texto siguen recibiendo la transcripción completa y pueden usar un fotograma que usted cargue.

<Diagram id="doc-block-cw43i6" title="Capturar, transcribir, reutilizar" summary={"Tres tipos de captura aterrizan en una biblioteca; el agente transcribe, titula y resume, luego cada transcripción se puede buscar y compartir."}>

```html
<div class="diagram-clips">
  <div class="diagram-col">
    <div class="diagram-node">Grabación de pantalla</div>
    <div class="diagram-node">Reunión de calendario</div>
    <div class="diagram-node">Dictado manteniendo Fn</div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    Una biblioteca<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">título · resumen · capítulos</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-pill">Buscar</div>
    <div class="diagram-pill">Compartir</div>
    <div class="diagram-pill">Legible por agentes links</div>
  </div>
</div>
```
...
- **Importa vídeos públicos de Loom.** En la web, selecciona **Grabar → Importar Loom** y pega una URL para compartir o incrustar de Loom. Clips guarda una copia en tu biblioteca e importa la transcripción pública cuando está disponible.
...
- **Vista previa de clips públicos en Slack** con un despliegue de vídeo reproducible después de que el
...
<Endpoint id="doc-block-1esmrg2" title="Punto de entrada del contexto del agente" method="GET" path="/api/agent-context.json" summary={"Descripción compacta y legible por el agente de un clip compartido"} auth="Same public / password / expiry rules as the share page" params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-evso54" title={"Transcripción con marca de tiempo"} method="GET" path="/api/agent-transcript.json" summary={"Transcripción con marca de tiempo segments for a shared clip"} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
<Endpoint id="doc-block-lqc8vu" title="Marco en una marca de tiempo" method="GET" path="/api/agent-frame.jpg" summary={"Un fotograma JPEG extraído del vídeo en una marca de tiempo del vídeo original."} params={[
  {
    "name": "id",
    "in": "query",
    "type": "string",
...
2. **Configurar el estado de producción.** Establecer un `DATABASE_URL` persistente, el normal
   ...
   proveedor de almacenamiento de vídeo. Builder.io Connect (free tier includes video storage and AI credits) es la ruta de almacenamiento más sencilla; para
...
6. **Conecta integraciones opcionales.** Google Calendar impulsa la pestaña Reuniones,
   `GEMINI_API_KEY` o Builder.io Connect (free tier includes AI credits) potencian la limpieza de transcripciones y títulos
...
<DataModel
  id="doc-block-1m84q58"
  title="Modelo de datos central Clips"
  summary={
  ...
  entities={[
    {
      ...
      fields: [
        ...
        },
        {
          name: "edits_json",
          type: "text",
          note: "Ediciones no destructivas",
        },
        {
          name: "chapters_json",
          type: "text",
        ...
        },
        {
          ...
          type: "text",
          nullable: true,
          note: "Privacidad: contraseña/caducidad",
        },
      ],
    },
    {
      ...
      note: "Divida para que las vistas de biblioteca y transcripción se procesen rápidamente",
      fields: [
        {
          name: "recording_id",
          type: "text",
...
Cada operación invocable por el agente es un archivo TypeScript en `templates/clips/actions/`, montado automáticamente en `POST /_agent-native/actions/:name` y ejecutable desde CLI como `pnpm action <name>`. Hay unas 100 acciones; las agrupaciones útiles:
