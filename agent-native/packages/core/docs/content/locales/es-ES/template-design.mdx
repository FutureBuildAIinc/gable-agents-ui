...
<WireframeBlock id="doc-block-1lq25vt">
  <Screen
    surface="desktop"
    html={
      "<div style='display:flex;flex-direction:column;gap:14px;padding:18px;min-height:520px;box-sizing:border-box'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>Página de lanzamiento del producto</h1><span class='wf-pill accent'>Desktop</span><span class='wf-pill'>Tablet</span><span class='wf-pill'>Mobile</span><div style='flex:1'></div><button>Preview</button><button class='primary'>Export code</button></div><div class='wf-card' style='flex:1;display:grid;grid-template-rows:auto 1fr auto;gap:12px'><div style='display:flex;gap:8px'><span class='wf-pill accent'>Hero</span><span class='wf-pill'>Pricing</span><span class='wf-pill'>FAQ</span></div><div class='wf-box' style='display:flex;flex-direction:column;gap:12px;min-height:230px;padding:20px'><div style='display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center'><strong style='font-size:22px'>Lanza más rápido con Agent-Native</strong><span class='wf-muted' style='max-width:70%'>Genera un prototipo funcional a partir de un prompt y después refínalo en el mismo lugar.</span><button class='primary'>Empezar</button></div><div style='display:grid;grid-template-columns:repeat(3,1fr);gap:10px;flex:1'><div class='wf-card' style='min-height:56px'></div><div class='wf-card' style='min-height:56px'></div><div class='wf-card' style='min-height:56px'></div></div></div><div class='wf-card' style='display:flex;align-items:center;gap:10px'><span class='wf-muted'>Haz el hero más denso y el CTA más claro.</span><div style='flex:1'></div><button class='primary'>Aplicar revisión</button></div></div></div>"
    }
  />
</WireframeBlock>
...
- **Empieza con plantillas reutilizables.** Elige un formato con dimensiones para anuncios, documentos de una página o páginas de destino, o guarda cualquier diseño integrado como plantilla. La indicación ajusta el contenido desbloqueado y conserva logotipos y fondos bloqueados.
...
La tabla siguiente cubre los recursos propios principales: diseños, archivos, plantillas, versiones y sistemas de diseño. Otras siete tablas del mismo esquema respaldan funciones descritas en otras partes de este documento, en lugar del modelo de recursos principal: `design_localhost_connections` y `design_localhost_write_grants` (modo de origen localhost), `component_index` (indexación de componentes), `motion_timeline` (movimiento), `design_state` (instantáneas del estado del editor), `design_fusion_edits` (modo de fusión de aplicaciones completas) y `design_review_snapshot` (instantáneas de revisión).
...
usuarios sin sesión pasan por `/sign-in?return=...`; después,
