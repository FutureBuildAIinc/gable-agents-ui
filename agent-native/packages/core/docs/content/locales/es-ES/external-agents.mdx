...
description: "Conecte Claude, ChatGPT, Codex, Cursor, Claude Cowork, Grok o cualquier host compatible con MCP a una aplicación nativa del agente alojada y luego envíe los artefactos de ida y vuelta al UI en ejecución con aplicaciones MCP y enlaces profundos."
search: "Claude Anthropic ChatGPT OpenAI Claude Código Codex Cursor Claude Cowork Grok xAI MCP Aplicaciones agente nativo conectar herramientas de agente local agentes externos"
---
...
<div class="xa-trip">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Herramienta MCP call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    La app produce un artefacto<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>El usuario hace clic en el enlace</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
  ...
  <div class="diagram-box" data-rough>
    Writes <code>navigate</code> app-state<br /><small class="diagram-muted"
      >La UI enfoca el artefacto</small
    >
  </div>
</div>
```
...
- **Su propio tiempo de ejecución detrás del chat Agent-Native:** consulte [Agent Surfaces](/docs/agent-surfaces#byo-agent) y [Native Interfaz de chat](/docs/native-chat-ui#byo-agent-runtimes) cuando un agente creado con otro marco debería impulsar `<AssistantChat runtime={...}>`.
...
- **Subagentes personalizados locales:** use [Recursos del Agente](/docs/agent-resources) cuando desee perfiles de agente personalizados dentro del propio espacio de trabajo nativo del agente.

## Fácil configuración {#easy-setup}

Cada aplicación Agent-Native expone una URL MCP canónica:

```text
https://<your-app>.agent-native.com/mcp
```

En la aplicación, abra **Settings → Integrations** y busque el nombre del host
que ya conoce: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok o xAI. El único resultado de host externo abre **Settings → MCP**
con la guía más adecuada seleccionada. Copie la **MCP server URL** y péguela en
el host de IA. La URL es la URL base de la aplicación desplegada más `/mcp`.
...
pestañas específicas del host para Claude, ChatGPT, Cursor, Claude Code, Codex, Grok y Otros.

<a id="access-tab"></a>

### La opción MCP dentro de la aplicación {#mcp-settings}

La configuración estándar de la aplicación incluye una pestaña **MCP** junto a
**Integrations**. Muestra la URL MCP canónica de la aplicación, una tarjeta de
agente A2A si está disponible y las mismas guías de configuración que la página
de conexión completa. Use **Settings → MCP** como punto de partida o abra
`/mcp/connect` directamente para el flujo completo y el token alternativo.
...
ChatGPT: las aplicaciones MCP personalizadas requieren un espacio de trabajo web
de ChatGPT compatible. Si tiene acceso, abra **Settings → Apps → Advanced
settings**, active el modo de desarrollador cuando sea necesario, elija **Create
app**, pegue la misma URL MCP, elija OAuth, escanee las herramientas, inicie
sesión con Agent-Native, apruebe los ámbitos solicitados y active la aplicación
en un chat. Los espacios Business, Enterprise y Edu admiten MCP completo,
incluidas las acciones de escritura; Pro admite lectura y obtención en modo de
desarrollador. Si no aparecen **Apps** o **Create app**, su plan o la política
del espacio de trabajo no permiten esta configuración.
...
`reconnect` encuentra cualquier entrada de configuración MCP cuyo URL termine en `/mcp` para el host dado y el cliente seleccionado (que coincida con URL independientemente del nombre del conector), luego actualiza o reemplaza el material de autenticación sin tocar su skills instalado ni volver a ejecutar el flujo de instalación completo. Pase la aplicación base URL (por ejemplo, `https://plan.agent-native.com`): se infiere el sufijo `/mcp`. La autenticación y la carga de herramientas son por cliente, así que reinicie/recargue ese cliente después; Codex necesita una nueva sesión antes de que aparezcan las herramientas recién cargadas.
...
<Diagram id="doc-block-1y1gjh2" title={"Dos niveles de catálogo"} summary={"Cada persona que llama obtiene el nivel compacto de forma predeterminada; la superficie completa de ~105 herramientas es solo opcional. La búsqueda de herramientas cierra la brecha para que nunca nada quede realmente oculto."}>
...
<div class="xa-tiers">
  ...
  <div class="diagram-card" data-rough>
    ...
    ><strong>~105 herramientas</strong
    ><small class="diagram-muted"
...
Este es un ejemplo genérico de la política. Analytics configura intencionadamente `authenticatedReads: "off"` y solo expone las actions revisadas de `connectorCatalog`.
...
Una aplicación que proporciona `resolveExecutionContext` sustituye explícitamente esta secuencia de identidad predeterminada. Su resolver debe rechazar de forma independiente a los autores no verificados, invitados, miembros externos y espacios de trabajo no vinculados antes de devolver un principal.

Antes de ejecutar el agente, un mensaje directo verificado de Slack se asocia a un miembro existente de la organización de Agent-Native y se guarda el vínculo de identidad entre el espacio de trabajo y el usuario. El contexto resultante del usuario y la organización carga los recursos, las instrucciones y las habilidades de ese usuario. De forma predeterminada, se rechaza a los miembros del espacio de trabajo cuyo perfil se obtuvo correctamente pero que no tienen correo electrónico o aún no pertenecen a la organización. Las aplicaciones pueden configurar explícitamente `allowAnonymousOrgScopedSlackDm: true` para ejecutar a esos miembros como un principal de servicio anónimo limitado a la organización: el mismo nivel de visibilidad para toda la organización que usan los canales compartidos, pero nunca acceso privado del usuario. El agente recibe una nota visible y Slack muestra una sola vez cómo obtener acceso personal. Esta opción nunca admite fallos de obtención del perfil, autores no verificados, invitados, miembros externos (Slack Connect) ni espacios de trabajo no vinculados a una organización; esos casos reciben una respuesta cortés de rechazo. Los canales compartidos de Slack usan un principal de servicio y no heredan los permisos privados de ningún participante. OAuth administrado y el manifiesto generado de la aplicación de Slack solicitan el ámbito de bot `users:read.email`. Las instalaciones existentes deben volver a conectarse o instalarse para conceder un ámbito recién añadido; las instalaciones antiguas con token de bot deben añadirlo manualmente en Slack. Sin él, los mensajes directos se rechazan salvo que la aplicación habilite explícitamente `allowAnonymousOrgScopedSlackDm`.
...
Los agentes externos obtienen las herramientas reales de creación/actualización de la aplicación —las mismas acciones con nombre que llama el agente interno—, así que cuando un usuario pide contenido nuevo (un diseño, una presentación, un formulario, un documento), redáctalo directamente y guárdalo con esas herramientas en lugar de delegar el trabajo mediante `ask_app`. Las acciones marcadas con `endsTurn: true`, como un formulario de preguntas dentro de la app que espera la respuesta del usuario en el chat de esa misma app, se ocultan de MCP, WebMCP y A2A a menos que `mcpTool: true` — consulta [endsTurn en Definir acciones](/docs/actions-defining). Las entradas y resultados de herramientas soportan hasta 500.000 caracteres en WebMCP, de modo que una pantalla, presentación o documento entero cabe en una sola escritura — consulta [WebMCP](/docs/webmcp). Las llamadas WebMCP locales a la página incluyen el id de la pestaña del navegador (el mismo valor de `getBrowserTabId()` que usa la UI — consulta [Conciencia del contexto](/docs/context-awareness)), de modo que la navegación y selección por pestaña se resuelven a la pestaña que el agente está controlando, incluso cuando está oculta.
...
| `ask_app({ app, message, async?, maxWaitMs? })`    | bucle de agente     | enruta una tarea en lenguaje natural al agente dentro de la aplicación de esa aplicación; el trabajo largo devuelve un `taskHandle` duradero y un `taskId` heredado para sondear con `ask_app_status` |
...
### Solicitudes de agente de larga duración

Cuando `ask_app` devuelva un `taskHandle`, pase ese valor opaco sin modificar a
`ask_app_status`; conserva la ruta original de la tarea aunque cambie el
descubrimiento de aplicaciones entre llamadas. Los argumentos anteriores `app`
y `taskId` siguen aceptándose durante la migración.

Los hosts y gateways MCP no comparten un único tiempo de espera largo para las
llamadas a herramientas; algunos aplican un límite de cinco minutos y otros lo
hacen configurable. No diseñe una integración de agente externo alrededor de
una `tools/call` bloqueante de diez minutos. `ask-agent` y `ask_app` confirman
el trabajo alojado rápidamente y luego devuelven un `taskHandle` duradero y un
`taskId` heredado cuando expira la espera en línea. Sondee con `ask_app_status`
reutilizando sin cambios los `poll.arguments` devueltos, especialmente el
`taskHandle`; el resultado incluye `pollAfterMs` como un intervalo apto
para hosts. La tarea debe crearse antes de que la primera respuesta pueda
...
- El punto final `/mcp` existente acepta ese token como cualquier otro portador (ver [MCP Protocol](/docs/mcp-protocol)): ni punto final nuevo, ni transporte nuevo.
...
- [WebMCP](/docs/webmcp): herramientas locales de la página, el límite de 500.000 caracteres para entrada/resultado, y la cabecera del id de pestaña del navegador.
...
- [Actions](/docs/actions-overview): definición de actions, `publicAgent`, GET / `readOnly`.
