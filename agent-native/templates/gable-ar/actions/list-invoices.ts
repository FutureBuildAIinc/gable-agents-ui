...
export default defineAction({
  description: "List invoices in gable, optionally filtered by status (UNPAID, PARTIAL, PAID, VOID, OVERDUE).",
  schema: withBranch({
    status: z
      .enum(["UNPAID", "PARTIAL", "PAID", "VOID", "OVERDUE"])
      .optional()
  ...
  readOnly: true,
  run: async (args, ctx) => {
    return await gable.get("/api/v1/invoices", {
