...
import type { VendorInvoice, APPayment, APAgingSummary, PaymentMethod } from '../../types/ap';
...
export class AccountsPayable extends LitElement {
    ...
    private async _handleLogBillSubmit(e: Event) {
        ...
        try {
            ...
            ToastService.show(err instanceof Error && err.message ? err.message : 'Failed to create vendor invoice', 'error');
        }
    }
    ...
    private async _handlePayVendorSubmit(e: Event) {
        ...
        try {
            ...
            ToastService.show(err instanceof Error && err.message ? err.message : 'Failed to process vendor payment', 'error');
        }
    }
    ...
    private async _handleApproveInvoice(invoiceId: string) {
        try {
            ...
            ToastService.show(err instanceof Error && err.message ? err.message : 'Failed to approve invoice', 'error');
        }
    }
                ...
                <div class="backdrop-blur-md bg-zinc-900/40 border border-white/5 rounded-xl p-5 space-y-3">
                    ...
                    <div class="grid grid-cols-5 gap-2 text-center">
                        <div class="bg-white/[0.02] border border-white/5 rounded-lg py-3">
                            ...
                            <p class="text-lg font-bold font-mono text-zinc-300 mt-1">${formatCents(agingBuckets.current)}</p>
                        </div>
                        <div class="bg-white/[0.02] border border-white/5 rounded-lg py-3">
                            <span class="text-[10px] text-zinc-500 uppercase font-semibold">31 - 60 Days</span>
                            <p class="text-lg font-bold font-mono text-yellow-400 mt-1">${formatCents(agingBuckets.past30)}</p>
                        </div>
                        <div class="bg-white/[0.02] border border-white/5 rounded-lg py-3">
                            <span class="text-[10px] text-zinc-500 uppercase font-semibold">61 - 90 Days</span>
                            <p class="text-lg font-bold font-mono text-orange-400 mt-1">${formatCents(agingBuckets.past60)}</p>
                        </div>
                        <div class="bg-white/[0.02] border border-white/5 rounded-lg py-3">
                            <span class="text-[10px] text-zinc-500 uppercase font-semibold">90+ Days</span>
                            <p class="text-lg font-bold font-mono text-rose-500 mt-1">${formatCents(agingBuckets.past90)}</p>
                        </div>
                        <div class="bg-white/[0.03] border border-emerald-500/20 rounded-lg py-3">
                            ...
                            <p class="text-lg font-bold font-mono text-emerald-400 mt-1">${formatCents(agingBuckets.total)}</p>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody class="divide-y divide-white/5">
                                    ...
                                    ` : this.payments.map(pmt => html`
                                        <tr class="hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-right font-mono text-emerald-400">${formatCents(pmt.amount)}</td>
                                            <td class="px-4 py-3 text-center">
                                ...
                                <tbody class="divide-y divide-white/5">
                                    ...
                                    ` : this.agingSummary.map(a => html`
                                        <tr class="hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-right font-mono text-zinc-300">${formatCents(a.current)}</td>
                                            <td class="px-4 py-3 text-right font-mono text-yellow-400">${formatCents(a.past_30)}</td>
                                            <td class="px-4 py-3 text-right font-mono text-orange-400">${formatCents(a.past_60)}</td>
                                            <td class="px-4 py-3 text-right font-mono text-rose-500">${formatCents(a.past_90)}</td>
                                            <td class="px-4 py-3 text-right font-mono text-emerald-400 font-bold">${formatCents(a.total)}</td>
                                        </tr>
                                    `)}
                                </tbody>
                            </table>
                                ...
                                <div class="border-t border-white/5 pt-4 flex justify-between items-center">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <span class="text-xl font-bold font-mono text-emerald-400">
                                            ${formatCents(this._estimatedTotalCents)}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            @change=${(e: Event) => { this.payMethod = (e.target as HTMLSelectElement).value as PaymentMethod; }}
                                            class="w-full bg-zinc-800 border border-zinc-700 text-white text-sm rounded-lg px-3 py-2 outline-none focus:border-emerald-500"
                                            required
                                        >
