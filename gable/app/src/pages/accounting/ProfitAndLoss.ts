    ...
    private _renderAccountLines(items: AccountLineItem[]) {
        ...
        return items.map(item => html`
            <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                ...
                <td class="px-4 py-2.5 text-right font-mono text-zinc-200">${formatStatementCents(item.amount)}</td>
            </tr>
        `);
    }
    ...
    private _renderSubtotalRow(label: string, amount: number, highlight = false) {
        return html`
            <tr class="border-b border-white/10 ${highlight ? 'bg-white/[0.03]' : ''}">
                ...
                <td class="px-4 py-2.5 text-right font-mono font-semibold ${highlight ? 'text-lg' : 'text-sm'} ${statementAmountClasses(amount)} border-t border-white/10">
                    ${formatStatementCents(amount)}
                </td>
            </tr>
        `;
    }
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl text-center">
                            <div class="py-4">
                                ...
                                <p class="text-2xl font-bold font-mono ${statementAmountClasses(report.total_revenue)}">
                                    ${formatStatementCents(report.total_revenue)}
                                </p>
                            </div>
                        </div>
                        <div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl text-center">
                            <div class="py-4">
                                ...
                                <p class="text-2xl font-bold font-mono ${statementAmountClasses(report.gross_profit)}">
                                    ${formatStatementCents(report.gross_profit)}
                                </p>
                            </div>
                        </div>
                        <div class="backdrop-blur-md bg-white/5 border ${report.net_income >= 0 ? 'border-emerald-500/30' : 'border-red-500/30'} rounded-xl text-center">
                            <div class="py-4">
                                ...
                                <p class="text-2xl font-bold font-mono ${statementAmountClasses(report.net_income)}">
                                    ${formatStatementCents(report.net_income)}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr class="font-bold ${report.net_income >= 0 ? 'bg-emerald-500/5' : 'bg-red-500/5'}">
                                        ...
                                        <td class="px-4 py-3 text-right font-mono text-lg ${statementAmountClasses(report.net_income)}">
                                            ${formatStatementCents(report.net_income)}
                                        </td>
                                    </tr>
                                </tbody>
