...
// HandleGetEscalationPolicy returns the customer's lumber-index
...
// HandleSetEscalationPolicy validates and persists the policy, then returns
// the reloaded row so the caller sees any server-side normalization.
