...
// IndexRefreshPreview is the dry-run impact of applying a new index value:
// how many open quotes/customers it touches and the total estimated exposure.
type IndexRefreshPreview struct {
	...
	TopCustomers             []IndexRefreshTopCustomer `json:"top_customers"`
}
...
//
// It deliberately does NOT register GET /api/v1/market-indices — that list
// endpoint is owned by EscalatorHandler, and re-registering it would panic the
// ServeMux on duplicate pattern.
...
// synchronously kicks the scanner. The scanner is bounded by the number of
// active escalators on the index — typically subsecond.
func (h *IndexAdminHandler) HandleRefresh(w http.ResponseWriter, r *http.Request) {
	...
	// Kick the scanner outside the transaction. A failure here doesn't fail
	// the request — value + history are committed and the safety-net cron
	// re-evaluates. Surface via scan_kicked so the UI can flag it.
	scanFailed := false
	if h.scanner != nil {
...
func (h *IndexAdminHandler) HandlePreviewRefresh(w http.ResponseWriter, r *http.Request) {
	...
	// Top 5 by exposure dollars (small N — selection sort).
