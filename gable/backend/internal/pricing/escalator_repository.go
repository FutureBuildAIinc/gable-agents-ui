...
// UpdateMarketIndex writes the value columns. It uses the caller's executor, so
// it participates in an enclosing RunInTx.
//
// This previously called r.db.Pool.Begin directly, which opened its own
// transaction and committed independently of any surrounding one. That silently
// broke IndexAdminHandler.HandleRefresh, whose whole point is to commit the new
// current_value and its market_index_history audit row together: a failing
// history insert rolled back only the history, leaving the index moved with no
// record of why. A single UPDATE needs no explicit transaction of its own.
