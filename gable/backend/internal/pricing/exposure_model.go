...
// SubjectForEvent maps an exposure event type to its eventbus subject. Events
// that don't drive a notification (DETECTED, BLOCKED) return "" and are not
// published.
func SubjectForEvent(t EventType) string {
	switch t {
	case EventFlagged:
		return eventbus.SubjectExposureFlagged
	case EventEscalated:
		return eventbus.SubjectExposureEscalated
	case EventAckRequired:
		return eventbus.SubjectExposureAckRequired
	case EventAcknowledged:
		return eventbus.SubjectExposureAcknowledged
	case EventCleared:
		return eventbus.SubjectExposureCleared
	default:
		return ""
	}
}
...
type PortfolioSummary struct {
	...
	ByCustomer           []PortfolioCustomerRow    `json:"by_customer"`
	BySalesperson        []PortfolioSalespersonRow `json:"by_salesperson"`
}
...
// ExposureNotification is the unified payload published to the event bus for
// every notify-worthy exposure event. The notifier (an eventbus subscriber)
// resolves recipient emails from SalespersonID / CustomerID rather than
// carrying them in the payload, keeping producers free of email concerns.
type ExposureNotification struct {
	EventType       EventType  `json:"event_type"`
	QuoteID         uuid.UUID  `json:"quote_id"`
	QuoteLineID     *uuid.UUID `json:"quote_line_id,omitempty"`
	QuoteShortID    string     `json:"quote_short_id"`
	CustomerID      uuid.UUID  `json:"customer_id"`
	CustomerName    string     `json:"customer_name"`
	SalespersonID   *uuid.UUID `json:"salesperson_id,omitempty"`
	SalespersonName string     `json:"salesperson_name,omitempty"`
