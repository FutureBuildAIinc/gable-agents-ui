...
type ExposureChecker interface {
	...
	QuoteIDForOrder(ctx context.Context, orderID uuid.UUID) (*uuid.UUID, error)
}
...
// QuoteIDForOrder resolves the source quote for an order, or nil if the order
// has none (walk-in counter orders) or doesn't exist.
func (c *PostgresExposureChecker) QuoteIDForOrder(ctx context.Context, orderID uuid.UUID) (*uuid.UUID, error) {
	var quoteID *uuid.UUID
	err := c.db.GetExecutor(ctx).QueryRow(ctx,
		`SELECT quote_id FROM orders WHERE id = $1`, orderID).Scan(&quoteID)
	if err != nil {
		...
		return nil, fmt.Errorf("quote id for order: %w", err)
	}
	return quoteID, nil
}
