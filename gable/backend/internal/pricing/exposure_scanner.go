...
func (s *ExposureScanner) evaluateOne(
	...
	threshold := 0.0
	if pe.ThresholdPctAtSnapshot != nil {
	...
	default:
		...
		case PolicyAutoEscalate:
			if ewc.CustomerAgreementSignedAt == nil {
				// Fallback to FLAGGED when an AUTO_ESCALATE customer lacks a
				// signed agreement — never silently mutate a contractual
				// price without the signed authorization on file.
...
func (s *ExposureScanner) publish(
	...
	notif := ExposureNotification{
		EventType:       eventType,
		QuoteID:         ewc.QuoteID,
		QuoteLineID:     ewc.Escalator.QuoteLineID,
...
// (each value already abs-rounded by the event-emitter). Kept package-private
// to make the rollup logic independently testable from the I/O orchestration
// in rollupQuote.
...
func (s *ExposureScanner) sumLatestExposurePerLine(ctx context.Context, quoteID uuid.UUID, activeLines map[uuid.UUID]struct{}) float64 {
	...
	// Walk events in order; keep the most recent non-CLEARED exposure_dollars
	// per line. Per-line values are already unsigned (math.Abs in
	// evaluateOne), so summing them is a straight positive-magnitude rollup.
	// Applying Abs only to the final sum would let mixed-direction line
...
func (s *ExposureScanner) writeAudit(ctx context.Context, ev *QuoteExposureEvent) {
	...
	// DETECTED events are too noisy for the audit log.
