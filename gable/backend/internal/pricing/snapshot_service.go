...
// injected into the quote service via WithSnapshotService in cmd/server/main.go.
//
// Behaviour:
...
func (s *SnapshotService) SnapshotQuoteLines(ctx context.Context, quoteID uuid.UUID) error {
	...
	for _, line := range q.Lines {
		...
		// Per-iteration copies so each escalator row carries its own pointers
		// (the frozen policy/threshold are identical, but distinct addresses
		// avoid aliasing surprises if a later change mutates one row).
		baseIndex := idx.CurrentValue
		recordedAt := now
		lineID := line.ID
		policyCopy := policy
		thresholdCopy := thresholdPct
		escalators = append(escalators, PriceEscalator{
			...
			PolicyAtSnapshot:       &policyCopy,
			ThresholdPctAtSnapshot: &thresholdCopy,
			EffectiveDate:          now,
