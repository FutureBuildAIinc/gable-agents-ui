...
-- Migration: 081_price_protection
...
-- All changes are additive; existing migrations are immutable.
-- Rollback lives at migrations/down/081_price_protection_down.sql (outside the
-- migrator's glob, applied by hand).

-- 1. Extend market_indices with a stable code + taxonomy + activation flag
...
-- Backfill index_code for the three rows seeded in migration 023
...
-- Any other pre-existing row (operator-created before this migration) gets a
-- deterministic synthetic code so the NOT NULL + UNIQUE below cannot fail.
UPDATE market_indices SET index_code = 'IDX_' || REPLACE(id::text, '-', '') WHERE index_code IS NULL;

-- Enforce NOT NULL + UNIQUE on index_code now that all rows have values
ALTER TABLE market_indices ALTER COLUMN index_code SET NOT NULL;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'market_indices_index_code_unique') THEN
        ALTER TABLE market_indices ADD CONSTRAINT market_indices_index_code_unique UNIQUE (index_code);
    END IF;
END$$;

CREATE INDEX IF NOT EXISTS idx_market_indices_active ON market_indices(is_active) WHERE is_active;

-- Seed the commodity-specific index taxonomy needed for the MVP
...
-- Backfill commodity_kind for the original three seeded rows
...
-- 2. Market index history (append-only time-series)
...
-- Seed category defaults for the lumber subtree (slugs from migration 049)
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'price_escalators_current_state_check') THEN
        ALTER TABLE price_escalators ADD CONSTRAINT price_escalators_current_state_check
...
-- 6. Extend customers with per-account escalation policy
...
BEGIN
    ...
    END IF;
    -- Industry-typical lumber thresholds sit between 1-10%. The 50% ceiling is
    -- intentionally conservative; beyond that a customer is effectively opting
    -- out of escalator-driven re-quote. Relax in a follow-up migration if a
    -- "stop-loss at 100%" model is ever needed rather than raising it here.
    ...
    END IF;
    -- AUTO_ESCALATE is contractually meaningful -- never allow it without a
    -- recorded signed agreement.
...
-- 7. Extend products with a per-SKU index override + commodity flag
...
-- Initial backfill: mark every product under the lumber.* subtree as commodity
...
-- 8. Extend quotes with a denormalized exposure rollup for fast list filtering
ALTER TABLE quotes
